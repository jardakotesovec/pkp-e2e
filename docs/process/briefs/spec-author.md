...
Deliverables:
1. The spec at `{{spec_path}}`, following TEMPLATE, all three apps from the start, its Coverage section written with the scenarios (one row per actor, state and setting, each ending in the scenario that runs it or a one-line why not) and the scenarios spent breadth first (TEMPLATE "Canonical scenarios", "Spending the tier"). Write it as two Writes (the body, then the footnotes and references): a single call that generates for more than five minutes outlives the prompt cache, and the next call rebuilds the whole context.
