...
Deliverables:
1. The spec at `{{spec_path}}`, following TEMPLATE, all three apps from the start, its Coverage section written with the draft (one row per actor, state and setting, each ending in the scenario planned for it or a one-line why not). The canonical scenarios are the plan only: a numbered title and its given each, spent breadth first (TEMPLATE "Canonical scenarios", "Spending the tier"), no steps; the text is composed after the claim-check fold (RUNBOOK step 7 "Scenarios"). Write it as two Writes (the body, then the footnotes and references): a single call that generates for more than five minutes outlives the prompt cache, and the next call rebuilds the whole context.
