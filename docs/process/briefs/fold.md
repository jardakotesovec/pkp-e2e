...
{{change_list}}     step 6: .reports/{{feature}}/claimcheck-merge.md, or with two or fewer chunks the chunk reports themselves; step 9: the test-findings file(s), block IDs listed
...
4. Re-read the Coverage section: a state or setting the change list introduced gets a row, `planned` or given a one-line why not (at step 6 the scenario writer fills "Runs in" after you; at step 9 no row is left blank). Run `node docs/process/lint/lint-spec.mjs {{spec_path}}` and fix to zero.
