...
4. Re-read the Coverage section last: a state or setting the change list introduced gets a row, `planned` or given a one-line why not (at step 5 the scenario writer fills "Runs in" after you; at step 8 no row is left blank). Run `node docs/process/lint/lint-spec.mjs {{spec_path}}` and fix to zero.
