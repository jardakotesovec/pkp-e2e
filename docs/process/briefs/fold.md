...
{{fold_log_path}}   .reports/{{feature}}/fold-log.md, step 6 only (an H feature's slices append to the same file); at step 9 there is no log
-->
...
- Evidence date for the footnotes: {{check_date}}. At step 9 the run is the evidence: a new register entry reads `Basis: test run`, and its footnote cites the run date and what the screen showed.
...
6. At step 6, write `{{fold_log_path}}`: one line per block saying what it became (rule text / register ID / footnote / marker / dropped, with the reason for any drop or downgrade); a "Suite-asserted claims touched" section (every changed claim a test could assert, from the merge or, with two or fewer chunks, from the chunk reports; each marked folded, softened or unchanged); a "left in `.reports/`" list; and a "Folded spans" section listing every passage you changed by section and rule, scenario or entry. At step 9 there is no log: return the register IDs added or changed.
...
Return (short): the fold-log path (step 6); the register's counts (🐞 / ❓ / ✅) and the IDs added or changed; the lint result and whether every declared line tags `no-screen`; anything dropped or downgraded (block IDs only).
