...
Size: the chunk is cut for about 40 browser calls (RUNBOOK step 3). Drive every checklist line even if it takes more; do not stop at a count.

Report: `{{report_path}}`, in spec-section order (the fold needs that). Header: chunk, apps driven, the declared no-screen lines by spec line number, calls used. Then one entry per checklist line or tight group: spec line number(s), verdict (holds / wrong / imprecise / undetermined), the screen evidence (snapshot file name, locator), and for anything not "holds" a block in the digest format of step 3b (`### {{chunk}}-<n> — one line, product voice` / Affects / Status: corrects | new | undetermined / Apps / Proposed / Evidence pointer). Facts only, no spec prose beyond the proposed line.
