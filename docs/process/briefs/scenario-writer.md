...
- The fold log, for what the evidence added or removed: {{fold_logs}}.

Task, saving after each scenario:
1. Spend the table by class as the TEMPLATE Coverage comment "Spending the table" says: no count; a state is covered where an editor, author or reviewer meets it in an ordinary week, in the cheapest form available (a bullet where a scenario passes by, a second seeded object inside an existing test, a scenario of its own, in that order), and left under "Budget" with that reason otherwise. Then compose each scenario in TEMPLATE's shape from the verified body: the given as one clause, the steps in execution order, each typed value named, each outcome quoted from the Rules, Fields or Side effects, one "Control:" sentence (a control that names the seeded context or its defaults is checked against `docs/process/seed-facts.md` and `docs/process/scenarios.md` first: a fresh context asks for keywords at "request"), the other side's effect read after the action. Every sentence is a fact the body already states; you add no claim. Where a step would need a fact the body does not hold, leave the step out and record the row under "Left out" with the reason word that fits, never a guess.
