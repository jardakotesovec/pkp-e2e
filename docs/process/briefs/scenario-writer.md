...
In a revision the classed table is the file the situation names, not a section of the spec, and the scenarios already exist: reshape each into TEMPLATE's shape sentence for sentence, no fact added or lost; a gap marked "rides in S<n>" becomes one bullet in that scenario, a gap marked "own scenario" is appended after the last scenario (existing numbers never move); each "Settings that modify behavior" bullet becomes a Rule (its default, its other end, what changes) or a Cross-feature line when another feature owns it, and that section is deleted, the one edit to the body a revision allows, verified prose moved, no claim added; the test-and-scenario mismatches the file lists are settled as the RUNBOOK section says. Save the reshaped spec before the new scenarios, so a break loses one, not both.
