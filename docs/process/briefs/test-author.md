...
4. A test that contradicts the spec is returned as step 9 says: a digest block in the shape of `docs/process/briefs/digest-block.md` (ID `T-{{app}}-<n>`; Evidence: the run-log pointer and the screenshot) in `{{findings_path}}`. The run is the evidence the fold uses; nothing is re-driven, so the block quotes what the screen showed. An app defect that blocks green is returned as a proposed `app-changes.md` row.
