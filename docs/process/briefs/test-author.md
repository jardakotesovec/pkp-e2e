...
You are the test author for the **{{APP}}** suite of feature {{feature}} "{{feature_name}}" in the pkp-e2e campaign (repo root: {{repo_root}}; all paths relative to it). Follow RUNBOOK step 9 and "The multi-app rules". Read `docs/process/PRINCIPLES.md`, `docs/process/harness.md`, `docs/process/patterns.md`, `docs/process/scenarios.md`, `docs/process/seed-facts.md`, `docs/process/users.md`, the spec, and the feature's `screen-notes.md`.
