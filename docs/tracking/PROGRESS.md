...
| U21 | Submission wizard | OJS OMP OPS | H | done | Spec verified. 13 tests OJS, 14 OMP, 14 OPS (+setup). Register 10🐞+7❓+2✅; headliner A8 (silent editor auto-assignment failure on every journal but the install's first). Open red: A11 regression (pkp-lib `9e2fbac214`), fix in pkp-lib PR #13265 e2e-verified, awaiting merge (ci-triage U21-A11). |
...
| U27 | Reviewer assignment & management | OJS OMP | H | done | Spec verified. 14 tests OJS, 15 OMP, 1 OPS absence (+setup). Register 28 entries: 14🐞+5❓+9✅, 8 retired; headliner A18 (silent half-add); 3 observations in the private file. Team triage 2026-08-29: A21 risk accepted, A22 ticket to follow. A25's scenario and tests are PARKED pending the upstream fix (maintainer request 2026-08-31). |
...
| U41 | Contributors & affiliations | OJS OMP OPS | M | done | Spec verified. 9 tests OJS, 11 OMP, 9 OPS (+setup). Register 10🐞+9❓+3✅; headliners A15 (new contributor ties the auto-created author at sequence 0, order varies between loads) and A14 (one-role journals cannot save any contributor). 1 observation in the private file, verified. |
| U42 | Citations & references | OJS OMP OPS | M | pending | |
| U43 | Funding | OJS OMP OPS | L | done | Spec verified. 5 tests per app (+setup). Register 4🐞+9❓+1✅; headliner A3 (a registry-picked funder saves nameless when the server cannot reach ROR); A10/A11 low-confidence, need a server with egress. Open red on OMP/OPS: A13, fixed on OJS, waiting for their ui-library bump (ci-triage U43-A13). |
