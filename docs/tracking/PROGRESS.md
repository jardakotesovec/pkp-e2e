...
| U43 | Funding | OJS OMP OPS | L | in_progress | 2026-08-28 on PINNED apps (local-only, no CI): ojs ac67a6dd76, omp 244a04311c, ops 94f6bbc59a. Steps 1–7 DONE: spec finalized+lint-clean (register 3🐞+8❓+1✅; headliner A3 registry pick saves a nameless funder, all apps); 5 tests+setup per app, full suites green ×2 fresh-reset (OMP had 3 load-flaked runs in between — PHP 30s-timeout fatals under load 13+, log-evidenced, different tests each time, clean under normal load). Step 8 claim check DONE (85 claims, 7 wrong, all folded + pending-fold corrections; register now 3🐞+9❓+1✅ — new A12 ❓ "required" marker unenforced; lint zero). REMAINING: post-fold confirm run of the 3 U43 suites (spec edits touched S2/S4 wording — tests expected to hold), step 9 readability, sign-off. Paused for account switch; RUNBOOK resume rules apply. |
