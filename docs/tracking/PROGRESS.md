...
| U29 | Review setup & review forms | OJS OMP | M | done | Spec verified. 10 tests OJS, 10 OMP, 1 OPS absence (+setup). Register 4🐞+5❓+2✅; headliners A4 (0 or empty in a deadline box silently becomes the 4-week / 3-week default in Add Reviewer), A6 (the Edit Review window opened on a request carrying a deactivated form drops the form on "OK"), A5 (the Review tab's written address lands on Submission after a reload), A7 ❓ (recommendation rows keep no fixed order after a toggle or edit). Shared ReviewSettingsPages POM (OJS adds the Recommendations tab, OMP the two guideline boxes). Low-confidence: A1 ❓ (after-due reminders, task runner off, code-read only). 11 minor readability frictions open (3 fact: F6, F7, F8). Medium-effort trial session. |
