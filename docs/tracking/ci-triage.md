...
|--------|--------|-------|-------|-----------------|
| pkp/ui-library#853 + pkp/ojs#5444 (submodule-only; no omp/ops PRs; issue pkp/pkp-lib#13359, reload tables only when a side modal changed something) | `optimize-table-reloads` | ready | 2026-09-23 | Prepared before merge at the maintainer's request; verdict: three findings (publish major in OMP/OPS, upload intention gap, Review Details mark-as-read race; reports `docs/reports/2026-09-23-ui-library-853*.md`), fix before merge advised for the publish one. Test changes: U14 `closeCommentPanel({changed})` in the three apps (view-only closes no longer reload), U23 S9 (OJS, OMP) waits for the mark-as-read before "Cancel"; specs U14, U23, U26, U27, U49 folded; kept checks `checks/sync/ui-library-853/`. At merge: rebase onto `main`, run the OJS suite at the merged tip (and OMP/OPS U14 and U23 at their tips), fast-forward, delete this row; the OMP/OPS publish reds arrive with their lib/ui-library update unless the PR fixes them first. |
