...
|--------|--------|-------|-------|-----------------|
| pkp/pkp-lib#13346 + pkp/ui-library#987 + pkp/ojs#5835 (submodule-only; no omp/ops PRs; issue pkp/pkp-lib#13263, review-date labels in History and Review Details) | `i13263` | ready | 2026-09-24 | Prepared before merge at the maintainer's request; CI green at the PR refs (dispatch 35967279054, ojs at the PR head, omp and ops at main); the ojs PR's red shards 2 and 3 (U27 S7, S9, U28 S2) are the intended History relabelling, accommodated here (OJS U27/U28 tests, U27/U28 specs, A29 half fixed); OMP's same three tests (U27 S7, S9, U28 S2 via `openHistory`) red once OMP's lib/pkp pointer carries #13346 (green locally at the merge result, red on CI at OMP `main`, dispatch 35965839857), so their edits are held back: `git revert` the companion commit "OMP History test edits held back" in the sync that moves OMP's pointer; no open findings: U27 A33, A27 and A35 ruled intended and A34 (translation lag) not a finding by @jarda.kotesovec 2026-09-24, all retired; kept check `shared/playwright/checks/sync/pkp-lib-13346/states.js`. Merge: rebase, run OJS and OMP U27/U28 (and the full suites if the tips moved), fast-forward, delete this row; the ojs PR's pointers predate ui-library #853 and a translations merge, so the developer should re-point them to the merge result before merging |
