...
| pkp/ui-library#853 (head `cab09538`, open; issue pkp/pkp-lib#13359; pre-merge, companion `optimize-table-reloads`) | Review Details window closed before its mark-as-read (`POST …/reviewAssignments/{id}/consider`) answers: the workflow Reviewers row stays "Review Submitted" with no reload, the dashboard list reloads at the close but keeps "View unread recommendation"; a page reload shows the truth (U27 ⚠ A32) | OJS (OMP and OPS with their next lib/ui-library update) | `checks/sync/ui-library-853/stale-read.js` (holds the POST until after the close; fixed when `.W.stale` and `.D.stale` are false; before at `2034439a`: both false) | 2026-09-23, `docs/reports/2026-09-23-ui-library-853.md` (thread + DMs) | 2026-09-23: narrowed, not closed, at the PR head `51f0c727` (the window is marked before the POST is sent, so a close while it is in flight now reloads, but that reload can run before the mark is saved, and a close before the POST is sent is not marked; the kept check still reads stale; the pre-companion U23 S9 red 2 of 16). Found as the PR check's U23 S9 red (run 35770880623); U23 S9 now waits for the mark before "Cancel", so no shipped test reds on it; not on `main` until the PR merges |
