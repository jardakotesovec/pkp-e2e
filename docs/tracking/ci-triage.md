...
|----|---------------------------|------|------------------------------|--------|-----------------------------|
| U21-A11 🐞 | **~100/129 OJS tests red at once**: every test that seeds a submission fails fast with `POST …/_test/scenarios/submission failed: 500` — `TypeError: Author::getAffiliations(): … null returned (Author.php:230)`; only U01/U04/U06-ish tests that never create a submission stay green | ojs (omp ops inherit at their next pkp-lib bump) | pkp-lib `9e2fbac214` made `getAffiliations()` throw on the null that `newAuthorFromUser()` stores for affiliation-less users — breaks the real wizard start too (`PKPSubmissionController::add()`), not just seeding — U21 register A11; reported to team 2026-09-01; upstream-ready report: `docs/reports/2026-09-01-author-getaffiliations-null-regression.md`. First red: pkp/ojs run 33536204412 at `d44b186c22` ("Submodule updates"). *Fix PR up*: pkp-lib#13265 (`62be01e1`, merge held on Travis) — e2e-verified 2026-09-01 at the PR ref: seeding + no-affiliation submission fixed, **but the with-affiliation branch now 500s** (`collect($affiliationObj)` casts the object's props to items — needs `collect([$obj])`); counter-bug reported to the author in-thread; *author revised same day* (`eb4cef9203`, `collect([$migratedAffiliation])`) — **e2e-verified**: both curl branches 200, migrated affiliation lands in `author_affiliations`, and the **full OJS suite 129/129 green, 0 flaky** at the PR ref (2026-09-01, env 0 — first fully green run on record; also verified the U49 S11 content-verified-save hardening). Note the suite alone can't catch the with-affiliation branch: every seeded user is affiliation-less | open — fix verified at PR ref, awaiting merge (Travis blocked by SFU fail2ban) + ojs pointer bump | 2026-09-01 / 2026-09-01 |
...
- **Decision-wizard timing under load** — U26 S5/S7/S8 (and U49 S11 +
  ...
  **Verified 2026-09-01**: green in the full 129/129 run at the pkp-lib
  #13265 PR ref (the run that cleared A11 locally) — hardening holds.
