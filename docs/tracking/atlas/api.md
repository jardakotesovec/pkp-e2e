...
| API-014 | ojs omp ops | `PKP\API\v1\contributorRoles\ContributorRoleController` | Contributor roles CRUD (base `contributorRoles`): GET {roleId}, GET /, GET identifiers, POST /, PUT {roleId}, DELETE {roleId}. Claimed by: contributors-and-affiliations. |
...
| API-033 | ojs omp ops | `PKP\API\v1\rors\PKPRorController` | ROR registry lookup (base `rors`): GET {rorId}, GET /, POST / (addOrEdit). Claimed by: contributors-and-affiliations. |
