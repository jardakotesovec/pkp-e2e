 ...
 *
 * OJS-local Page Objects for the Stage participants feature (spec:
 * docs/specs/U35-stage-participants.md): the workflow's "Participants"
 * panel and its rows, the "Assign Participant", "Edit Assignment" and
 * "Notify" legacy windows, the "Remove Participant" dialog, the panel's
 * "Logout as" entry, the stage's discussions panel and a discussion's
 * window, and the settings forms scenario 6 flips (the Roles form's
 * recommend-only box, a section's "Editorial Assignments" boxes, the
 * "Editor Assigned (Auto)" template's subject).
 *
 * The workflow frame (opening, the side menu, the Activity Log) is the
 * shared `WorkflowPage`, held as `frame`. "Login As" is
 * `LoginSessionsPages.LoginAsDialog`. The row helpers of
 * `ReviewStagePages.js` (`participantRow`, `participantMoreActions`,
 * `openRowMenu`, `clickRowAction`) stay there for the suites that import
 * them; this file reads the panel through CSS anchored on
 * `[data-cy="participant-manager"]`, because for about 450 ms after an
 * inner legacy window closes the workflow dialog is aria-hidden and a
 * role query finds no row (U35 claim check, screen-notes 2026-09-20).
 *
 * Strings are the live locale strings (lib/pkp/locale/en/*.po); the DOM
 * shapes come from the U35 claim check's kept scripts
 * (shared/playwright/checks/U35) and were confirmed by this suite's run.
 */
...
/** The stage entries' menu keys. */
const MENU_KEY = {submission: 'workflow_1', review: 'workflow_3', copyediting: 'workflow_4', production: 'workflow_5'};
...
exports.ParticipantsPanel = class ParticipantsPanel extends BasePage {
    ...
    constructor(page, contextPath) {
        ...
        /** The shared workflow frame (opening, side menu, Activity Log). */
        this.frame = new WorkflowFrame(page, contextPath);
    }

    /** Open the workflow on the editorial dashboard at a stage entry ("Submission", "Copyediting", …). */
    ...
    /** Open the workflow at a review round's entry ("Review Round 1"). */
    async gotoRound(submissionId, roundId, round = 1) {
        await this.frame.gotoEditorial(submissionId, {menuKey: `${MENU_KEY.review}_${roundId}`});
        await this.frame.expectHeading(`Workflow: Review (Round ${round})`);
    }
    ...
    /**
     * Watch the workflow's own API fetches for this submission (`GET
     * …/api/v1/submissions/{id}…`) from now on; `settled()` waits until none
     * is in flight, the participants list's fetch has been refused (401) and
     * the count of answered fetches has stopped moving. A participant who
     * removes their own row triggers six refetches (the submission, its
     * publication, two file lists, the tasks and the participants), each
     * refused and each reopening the one "Error" dialog the modal store
     * holds, so an "OK" pressed before the last answer lands is followed by
     * a fresh "Error" (the OJS final of 2026-09-20, S2 red under four
     * workers); the caller dismisses the dialog after `settled()`.
     */
    watchSubmissionFetches(submissionId) {
        const page = this.page;
        const pattern = new RegExp(`/api/v1/submissions/${submissionId}(/|\\?|$)`);
        const isFetch = (request) => request.method() === 'GET' && pattern.test(request.url());
        const state = {inFlight: 0, answered: 0, participantsRefused: false};
        ...
        const onResponse = (response) => {
            if (isFetch(response.request()) && response.status() === 401 && /\/participants\//.test(response.url())) {
                state.participantsRefused = true;
            }
        };
        ...
        return {
            ...
            async settled() {
                ...
                await expect
                    .poll(
                        () => {
                            const quiet = state.inFlight === 0 && state.participantsRefused && state.answered === seen;
                            seen = state.answered;
                            return quiet;
                        },
...
exports.AssignParticipantWindow = class AssignParticipantWindow extends BasePage {
    ...
    /** The row names as listed. */
    async personNames() {
        return this.personRows().evaluateAll((rows) =>
            rows.map((tr) => {
                const cell = tr.querySelector('td:nth-child(2), td.first_column + td') || tr.querySelectorAll('td')[1];
...
exports.NotifyWindow = class NotifyWindow extends BasePage {
    ...
    /**
     * The refusal "Notify" answers with "Message" empty: a top-right toast
     * (`NotificationsPages.toasts`), not text inside the window, which stays
     * as it was (this suite's run, finding T-ojs-3).
     */
    emptyMessageToast() {
        return this.page.locator('[role="status"].app__notifications .pkpNotification').filter({hasText: NOTIFY_EMPTY_MESSAGE});
    }
...
// ---------------------------------------------------------------------------
// Settings › Journal › Sections › a section's "Editorial Assignments" (Rule 11b)
// ---------------------------------------------------------------------------

exports.SectionSettingsPage = class SectionSettingsPage extends BasePage {
    /**
     * @param {import('@playwright/test').Page} page
     * @param {string} contextPath
     */
    ...
    /** Settings › Journal, the "Sections" tab, its grid rendered. */
    async goto() {
        await this.page.goto(this.contextUrl(this.contextPath, '/management/settings/context'));
        await this.page.getByRole('tab', {name: 'Sections', exact: true}).click();
        await expect(this.grid().locator('tr.gridRow').first()).toBeVisible({timeout: 30_000});
        await waitForJQueryIdle(this.page);
    }

    grid() {
        return this.page.locator('#sectionsGridContainer');
    }

    form() {
        return this.page.locator('form#sectionForm');
    }

    /** Open a section's form by its title. */
