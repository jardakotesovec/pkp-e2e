 ...
 *
 * Notify users (bulk email) — OJS suite, the parallel part: scenario 3 (the
 ...
 *
 * Seeding (footnote s): a scratch journal from `POST scenarios/context` with
 * `bulkEmails: true` (the journal ticked under "Bulk Emails") and throwaway
 * accounts; the Site Administrator is `admin`, whom the context factory
 * enrols as a manager of every scratch journal. "Author" is ticked under
 * "Disable Roles" on the screen, as the scenario does; the change is the
 * journal's own, so it runs beside other workers. No test saves
...
const DISABLE_DESCRIPTION =
    'A journal manager will be unable to send bulk emails to any of the roles selected below. Use this setting to ' +
    ...
    'can be disabled completely for this journal in Admin > Site Settings.';
...
test.describe('notify users', () => {
    test('S3: the Site Administrator takes a role off the "Notify" tab', async ({asUser, ojsApi}, testInfo) => {
        test.slow();
        ...
        // The journal's roles as its "Roles" tab lists them (the order the
        // side tab follows).
        ...
        // The Journal Manager: "Author" not offered, every other role is,
