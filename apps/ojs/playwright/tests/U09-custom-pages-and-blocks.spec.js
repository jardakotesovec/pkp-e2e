 ...
 *
 * Custom pages & blocks — OJS suite, one test per canonical scenario the
 * spec runs on a journal (scenarios 1–6, common, and 7, {OJS OMP});
 * scenario 8 is the preprint server's absence scenario (OPS suite).
 ...
 *
 * Seeding: scenario endpoints only. Every scenario runs on a scratch
 * journal with a throwaway Journal Manager (footnote y): S3 adds a Reader
 * and an Editor whose role is seeded with "Permit changes to Settings"
 ...
 * installs (A14), so each block lives on its test's own journal. S5's
 ...
 * its own `asUser` context. Everything runs in the parallel `ojs` project.
 */
...
function makeTag(scenario, testInfo) {
    return `u9${scenario}ojw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
/** A bare "404 Not Found" page: the status, the words, no journal header. */
...
test.describe('custom pages & blocks', () => {
    test('S1: a custom page, from the item window to the visitor', {tag: '@smoke'}, async ({asUser, ojsApi, visitor}, testInfo) => {
        test.slow();
        ...
        // both tags replaced, inside the journal's header and footer; the
        ...
        // Control, while the page stands: the Journal Manager, signed in,
        // sees the same page with no "Edit" link, where "About the Journal"
        // offers the manager one (Rule 2).
    ...
    test("S2: a custom page on a built-in page's address", async ({asUser, ojsApi, visitor}, testInfo) => {
        test.slow();
        ...
        // The header's "About" › "About the Journal" opens the custom page;
        // so does {journal}/about (Rule 6).
        await chrome.reload();
        ...
        // An address under it: the journal's contact page (Rule 6a).
    ...
    test("S3: a preview's address typed by hand", async ({asUser, ojsApi, visitor}, testInfo) => {
        test.slow();
        ...
        // The preview page with nothing typed: its tab reads "| {journal name}".
        const emptyPreviewTitle = new RegExp(`^\\s*\\|\\s*${name}\\s*$`);
        const anyEmptyPreviewTitle = /^\s*\|/;

        // Control first, the same read: the Journal Manager at the typed
        // address gets the journal's page with no title and no content
        // (Actors row 2).
        ...
        // The Editor without Settings: the journal's page with no title and
        // no content, as the manager's (Actors row 2).
    ...
    test('S4: a custom block, from the plugin to the sidebar', async ({asUser, ojsApi, visitor}, testInfo) => {
        test.slow();
        ...
        // the name a heading for screen readers only; "About the Journal"
    ...
    test('S5: pictures in a page, refused files and the allowance', async ({asUser, ojsApi, visitor, baseURL}, testInfo) => {
    ...
    test('S6: a journal in two languages', async ({asUser, ojsApi, visitor}, testInfo) => {
        test.slow();
    ...
    test('S7: a static page, from the plugin to the visitor', async ({asUser, ojsApi, visitor, baseURL}, testInfo) => {
        test.setTimeout(300_000);
        ...
        // under "Path" and "Title" (Fields), on this one-language journal
        // with no language in the address (T-ojs-1; the spec's line has
        // "/en").
        ...
        // journal's header and footer; the window still open, the list still
        // empty (Rules 7, 13).
        ...
        let answer = await withNoticeFetch(page, () => win.save());
        expect(answer.status).toBe(200);
