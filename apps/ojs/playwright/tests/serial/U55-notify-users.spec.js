 ...
 *
 * Notify users (bulk email) — OJS suite, the serial part: scenarios 1, 2, 4
 ...
 * `@solo`: its Site Administrator unticks the journal on Administration ›
 ...
 * journal another worker seeds between that page's load and its "Save"
 * would be unticked with it. Alone in the `ojs-solo` project nothing seeds
 ...
 *
 * Seeding (footnote s): a scratch journal from `POST scenarios/context`
 * with `bulkEmails: true` and throwaway accounts (the username twice as
 * password, `<username>@mail.test`), the first a Journal Manager. The
 * Site Administrator is `admin`, enrolled as a manager of every scratch
 * journal. S1 names the journal's principal contact, so the sender the
 * email carries is the test's own and is read back on Settings › Journal ›
 * "Contact". Every actor is opened through `asUser` (patterns.md "Fixture
...
const WIZARD = {firstTab: 'Journal Settings'};
...
const NOT_ENABLED = 'The email notification feature has not been enabled for this journal.';
const DISABLED_SENTENCE =
    'The bulk email feature has been disabled for this journal. Enable this feature in Admin > Site Settings.';
/** "Closes itself after about five seconds": bounded well above that. */
const NOTICE_LIFETIME = 15_000;

/** Unique per-run tag: single alphanumeric token, app + scenario + worker. */
function makeTag(scenario, testInfo) {
    return `u55s${scenario}ojsw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
test.describe('notify users (serial)', () => {
    test('S1: send an email to one role, then to two with a copy', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        ...
        // Control: Rui and the Journal Manager have no "Office closed".
        for (const who of [rui, manager]) {
            await pkpMail.expectNone({to: mail(who), subject: 'Office closed', afterControl: {to: mail(quinn), subject: 'Office closed'}});
        }
    });

    test('S2: the roles offered, and a form refused, cancelled and left', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        ...
        await notify.settings.openSettingsEntry('Journal');
        await expect(notify.settings.heading).toHaveText('Journal Settings');
    ...
    test('S4: a role restricted while the manager\'s page is open', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.setTimeout(180_000);
    ...
    test('S5: bulk email withdrawn while the manager\'s page is open @solo', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.setTimeout(180_000);
        ...
        // The Site Administrator unticks the journal (Settings bullet 1).
        ...
        // "Restrict Bulk Emails" on a journal not allowed (Rule 11).
        const hosted = new HostedContextsPage(ap, LABELS);
        await hosted.gotoFromAdministration();
        await hosted.openSettingsWizard(tag);
        const restrict = new RestrictBulkEmailsTab(ap, WIZARD);
