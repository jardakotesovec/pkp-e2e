 ...
 *
 * Navigation menus & site chrome — OJS suite, the serial part: S8 (the
 ...
 * the `ojs-solo` project after the serial one (harness.md "Project
 ...
 *
 * Seeding: one scratch journal through `POST scenarios/context`, so the site
 * hosts two or more journals and offers the "Navigation" side tab (Rule 1b)
...
test.describe('navigation menus & site chrome (the site)', () => {
    test("S8: the site's own Navigation tab @solo", async ({asUser, ojsApi, visitor}, testInfo) => {
        test.slow();
        await ojsApi.createContext({tag: makeTag('8', testInfo)});
