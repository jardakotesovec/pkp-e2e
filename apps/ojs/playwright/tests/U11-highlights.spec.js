 ...
 *
 * Highlights — OJS suite, one test per canonical scenario (S1–S4, all
 * common; the feature has no app-specific scenario).
 ...
 * the panel's or the carousel's heading), A8 ❓, A9 ❓ (S1 replaces no
 * image). The spec's Coverage section records everything else left out.
 *
 ...
 * journal's public files directory, never the rendered picture. Signed-out
 ...
 * a mail the test sends the same way, the manager's "Notify" on a seeded
 * submission (A8, M4); the Tasks silence is the bell and the window's "No
 * Items", bounded by the seeded submission's own task (M6). The slide
 * button's address is followed against a routed stand-in for example.org,
 * so no test reaches an outside address. Waits are event-based (highlights
 * API responses, temporary-file uploads, web-first assertions) — no
 * hard-coded sleeps. Everything runs in the parallel `ojs` project.
 */
const fs = require('fs');
...
function makeTag(scenario, testInfo) {
    return `u11${scenario}ojw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}

/** A throwaway account's address (users.md: `<username>@mail.test`). */
const mailOf = (username) => `${username}@mail.test`;

/** A throwaway account for the scratch journal. */
...
/**
 * The journal's public highlights directory on disk (footnote g:
 * `<public_files_dir>/journals/<id>/highlights`), read from the app's test
 * config the way the harness reads it.
 */
function highlightsDir(contextId) {
    ...
    return path.join(pub, 'journals', String(contextId), 'highlights');
}
...
test.describe('highlights', () => {
    test('S1: add, edit and delete a highlight', {tag: ['@smoke']}, async ({browser, baseURL, asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
        ...
        // page's own content (the main block is on screen).
        await tab.goto();
        ...
        await expect(home.imageOf(home.slide(0))).toHaveCount(0);
        // The stored file (Rule 10): the slide's picture is the journal's
        ...
        // "No Items"; then a submission seeded on the journal is the
        // positive control, the same bell and window carrying its task.
        ...
        // No email arrived from the saves and deletes: the manager's
        // "Notify" to the spare on the seeded submission is the one mail
        // sent the same way and bounds the read; the manager's and the
        // three other throwaway addresses hold nothing.
        const pub = new PublicationScreen(page, tag);
        await pub.gotoWorkflow(submissionId);
        await pub.openStage('Submission');
        await pub.notifyParticipant('Xena Spare', `<p>Control ${tag}</p>`);
        ...
        // Control: Section Editor, Author and Reader each get the
        // access-denied page at the address the manager used (Actors row
        // 1); the manager's own visit above is the positive control.
        for (const username of [sectionEditor, author, reader]) {
    ...
    test('S2: reorder the highlights', async ({browser, baseURL, asUser, ojsApi}, testInfo) => {
        test.slow();
    ...
    test('S3: read the carousel as a visitor', async ({browser, baseURL, asUser, ojsApi}, testInfo) => {
        test.slow();
        ...
        // Control: the seeded journal's home page shows none of these
        // slides (Rule 1); the scratch journal's carousel above is the
        // positive read.
        await gotoHome(visitor, JOURNAL, 'en');
    ...
    test('S4: highlights in a second language', async ({browser, baseURL, asUser, ojsApi}, testInfo) => {
        test.slow();
