 ...
 *
 * Tasks & discussions — OJS suite, one test per canonical scenario the spec
 * runs on OJS (the common scenarios 1–9 and the journal-and-press scenarios
 * 10 and 11).
 ...
 *   its "Edit".
 * - OMP1: S11 reads the journal's "Reviewer".
 * - A1 🐞, A4 🐞, A7 🐞, A8 🐞, A9 🐞, A10 🐞, A16 🐞, A17 🐞, A24 🐞,
 *   A29 🐞, A31 🐞, A12 ❓, A14 ❓, A15 ❓, A19 ❓, A23 ❓, A27 ❓, A30 ❓:
 *   no scenario reaches them here.
 ...
 * journal, whose submissions arrive with their section's editors assigned
 * (`sectioneditor.ana`, `sectioneditor.omar`, `editor.diana`); S8, S9 and
 * the "Open" half of S11 run on scratch journals with throwaway accounts.
 * Items seeded in one call share a second, so a row is always found by its
 * name, never by its position. The roster's mailboxes are shared by every
 * worker (A8): a mail is picked by its recipient, its subject and the
 * submission's own footer mark ("#{id} Author", every discussion email's
...
const {clickRowAction, legacyModal, statusTitle} = require('../pages/ReviewStagePages.js');
const {
    TEXT,
...
const reviewKey = (roundId) => `workflow_3_${roundId}`;
...
function makeTag(scenario, testInfo) {
    return `u37${scenario}ojsw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
async function panelAs(asUser, username, submissionId, stage, {contextPath = JOURNAL, menuKey = null} = {}) {
...
async function authorPanelAs(asUser, username, submissionId, stage, {contextPath = JOURNAL, menuKey = null} = {}) {
    const page = await pageFor(asUser, username);
    const panel = new TasksDiscussionsPanel(page, contextPath, {title: PANEL[stage]});
...
test.describe('tasks and discussions', () => {
    test('S1: open a discussion and reply', {tag: '@smoke'}, async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
        const tag = makeTag('s1', testInfo);
        const title = `Submission ${tag}`;
        const {submissionId} = await seed(ojsApi, tag, {decisions: TO_PRODUCTION, files: [{file: 'article.pdf'}]});
        ...
        await sePage.goto(`/index.php/${JOURNAL}/dashboard/editorial`);
        ...
        const se = {page: sePage, panel: new TasksDiscussionsPanel(sePage, JOURNAL, {title: PANEL.production})};
    ...
    test('S2: what the "Add" window refuses, and "Cancel"', async ({asUser, ojsApi}, testInfo) => {
        test.slow();
        const tag = makeTag('s2', testInfo);
        const {submissionId} = await seed(ojsApi, tag, {decisions: TO_PRODUCTION});
    ...
    test('S3: templates in the "Add" window', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test('S4: a task: begun, started, closed, overdue', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test('S5: close, reopen, and turn a discussion into a task', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test('S6: edit a discussion, read its History, delete it', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test('S7: the Author\'s discussions', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test('S8: manage the templates', async ({asUser, ojsApi}, testInfo) => {
        test.slow();
        ...
        // The screen: the table, its line, the stage groups with "Add
        // template", and the installed Production templates, boxes unticked.
        ...
        await expect.poll(() => templates.groupLabels()).toEqual(['Submission Stage', 'Review Stage', 'Copyediting Stage', PRODUCTION_STAGE]);
        for (const stage of ['Submission Stage', 'Review Stage', 'Copyediting Stage', PRODUCTION_STAGE]) {
    ...
    test('S9: "Auto-add at stage"', async ({asUser, ojsApi}, testInfo) => {
        test.slow();
    ...
    test('S10: the Copyeditor\'s task', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test('S11: reviewers and the Author on a review stage', async ({asUser, ojsApi, pkpMail}, testInfo) => {
        test.slow();
        ...
        const roundKey = reviewKey(seeded.reviewRounds[0].id);

        // Who the Section Editor is offered: themselves, the Author, both
        // Reviewers with their round line and "Reviewer".
        ...
        for (const reviewer of [REVIEWER, SECOND_REVIEWER]) {
            ...
            await expect(add.participantLabel(reviewer)).toHaveText(new RegExp(`\\(${esc(reviewer)}\\)\\s*${ROLE.reviewer}\\s*${esc(ANONYMOUS)}`));
        }
        ...
        const wizard = new ReviewWizardPage(rvPage, JOURNAL);
        ...
        const rv = new TasksDiscussionsPanel(rvPage, JOURNAL, {title: PANEL.review});
        ...
        await se.panel.reland();
        const reviewerRow = se.panel.frame.panel('Reviewers').getByRole('row').filter({hasText: NAME[REVIEWER]});
        await clickRowAction(se.page, reviewerRow, 'Cancel Reviewer');
        const cancelModal = legacyModal(se.page, 'cancelReviewForm');
        ...
        await cancelModal.locator('form#cancelReviewForm').getByRole('button', {name: 'Cancel Reviewer', exact: true}).click();
        await expect(cancelModal.locator('form#cancelReviewForm')).toBeHidden({timeout: 30_000});
        await se.panel.reland();
        await expect(statusTitle(reviewerRow)).toHaveText('Request Cancelled');
        await se.panel.expectInGroup(QUESTION, 'In progress');
        ...
        // An "Open" review on a scratch journal.
        ...
        const open = await seed(ojsApi, openTag, {
            context: openTag,
            submitter: p.au.username,
            participants: [{username: p.se.username, role: 'sectionEditor'}],
            decisions: ['sendExternalReview'],
        ...
        const openKey = reviewKey(open.reviewRounds[0].id);
        ...
        // Control: the seeded journal's Author was offered no Reviewer (above),
