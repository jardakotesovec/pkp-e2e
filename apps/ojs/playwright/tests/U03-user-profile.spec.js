 ...
 * OJS (scenarios 1–10; the spec lists no OJS-specific scenario).
 ...
 *   one-journal user): every test opens the journal-level page; the
 ...
 * - A4 🐞 (a closed journal keeps an empty section on the Roles tab): the
 *   registration setting is not touched.
 * - A5 ❓ (the scheduled-role banner ignores which journal is open) and Rule
 ...
 *   author out of My Submissions): S6 ticks and unticks a role the user
 *   took themselves and never asserts the lock-out.
 ...
 * notification choices are touched (PRINCIPLES A7). Every sign-in happens in
 * the test's own fresh browser context through the real Login form, never
 * through the shared .auth cache, because a password change ends the
 * account's other sessions and a sign-out ends the cached one; the only
 * cached identity used is `admin` (a scratch journal's auto-enrolled
 * manager) for the Users & Roles reads. Mailpit reads are scoped by the
 * throwaway recipient (PRINCIPLES A8) and every silence claim has a positive
 * control. A refused image upload raises a browser alert (Rule 9a), so S7
 * registers a dialog handler before its upload. No hard-coded waits.
 */
...
/** Users & Roles (Users tab) of a scratch journal, as its auto-enrolled manager `admin`. */
...
test.describe('user profile', () => {
    test('S1: reach the profile and its tabs', {tag: '@smoke'}, async ({browser, baseURL}) => {
        ...
        try {
            ...
            const profile = new ProfilePage(page, JOURNAL);
    ...
    test('S2: rename yourself and change your initials', async ({browser, baseURL, ojsApi, asUser}, testInfo) => {
        test.slow();
        ...
        try {
            ...
            // (Rule 4; `admin` is the scratch journal's auto-enrolled manager).
    ...
    test('S3: update contact details', async ({browser, baseURL, ojsApi}, testInfo) => {
        ...
        try {
            ...
            // right and in the box's label; the phone change is lost.
    ...
    test('S8: change the password', async ({browser, baseURL, ojsApi}, testInfo) => {
        test.slow();
        test.setTimeout(180_000);
