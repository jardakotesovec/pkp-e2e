 ...
 *
 * Reader comments & moderation — OPS suite: one test per canonical
 * scenario the spec runs on a preprint server (S1–S7 common, S14
 * {OMP OPS}), in the server's own vocabulary: Preprint Server Manager,
 * Moderator (the OPS `sectionEditor`), preprint, the preprint's landing
 * page (`preprint/view/{id}`), and a "Content" group that exists only
 * while public comments are on and then holds "Comments" alone (Rule
 * 3c). A preprint server installs the moderation half only, so nothing
 ...
 * page bullets ({OJS}) are left out here. A preprint server has no
 * `editor` user-group key (users.md), so where a common scenario names an
 * Editor the manager is the only moderator level here (footnotes s0, s5):
 * S5 reads the manager's panel and the Moderator's as the control, and
 * has no "Editor's Tasks panel" bullet ({OJS OMP}). S14 reads the
 * preprint's landing page for the absence of any comments block, with the
 * "Comments" settings tab and the Comments page reading "No Items" as the
 * controls (multi-app rule 3).
 ...
 * manager role is never driven), OPS1 ❓ (the same: never driven). Where
 * a scenario marks a ❓ the test asserts that bullet's own sentence and
 * nothing of the register entry beyond it: S1 waits for the Comments
 * tab's reload onto Appearance › Theme and asserts nothing about a
 * "Saved" status (A5); S1 opens the Comments page by address with the
 * setting off and reads the kept comment (A4); S3 reads the address
 * after the report panel's "Close" with neither number (A8); S5 reads
 * the "pending review" row still in the manager's panel after "Hide
 * Comment" (A2). The spec's Coverage section records everything else
 * left out.
 *
 * Seeding: scenario endpoints only. Every test seeds its own scratch
 * preprint server (the context passthrough `enablePublicComments: true`,
 * throwaway accounts whose addresses name app + test,
 * u14s<n>opsw<i>…@mail.test) and a posted preprint carrying its comments
 * and reports through `userComments[]` (footnotes s0–s7, s14).
 * publicknowledge and the seeded users are never changed (A1, A7). A
 * batch seeded in one call shares a second and lists in no fixed order,
 * so rows are located by their "Comment" cell, never by position (M5).
 * S2's affiliation has no seed key and is set on the writer's Profile ›
 * Contact (Country first). Every absence is a settled read paired with a
 * positive control taken the same way (M4, M6): S5's mailbox silence is
 * an exact count of one after a control email the test sends to the same
 * address from Users & Roles › the row's "Email" (A8), its Moderator's
 * panel is read after the manager's showed the effect; S1's and S14's
 * missing "Content" group is read beside the "Statistics" group the same
 * menu keeps; S14's absences sit beside the page's own headings and the
 * signed-in name. Waits are event-based (the approval, delete and list
 * responses, the settings reload, web-first assertions) — no hard-coded
...
function makeTag(scenario, testInfo) {
    return `u14${scenario}opsw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}

/** A scratch server user spec; its address names app + test. */
...
/**
 * The standard cast: a Preprint Server Manager and a submitting author,
 * plus the readers asked for and, when asked, a Moderator (the OPS
 * `sectionEditor`); a preprint server has no `editor` key.
 */
function cast(tag, {readers = 0, sectionEditor = false} = {}) {
    const readerNames = [
        ['Rosa', 'Reader'],
        ['Rob', 'Reporter'],
        ['Rita', 'Third'],
    ...
    return {
        ...
        sectionEditor: sectionEditor ? scratchUser(tag, 'se', 'Sam', 'Moderator', ['sectionEditor']) : null,
        readers: readerNames.slice(0, readers).map(([given, family], i) => scratchUser(tag, `rd${i + 1}`, given, family, ['reader'])),
    };
}

function usersOf(people) {
    return [people.manager, people.author, people.sectionEditor, ...people.readers].filter(Boolean);
}

const fullName = (user) => `${user.givenName} ${user.familyName}`;

/** A scratch preprint server with public comments on (or off when `comments` is false). */
async function createServer(opsApi, tag, people, {comments = true} = {}) {
...
/** A posted preprint of the server carrying `userComments`. */
async function postPreprint(opsApi, tag, people, {title, userComments = []} = {}) {
...
/** The editorial dashboard of a server (the side menu and the Tasks bell live there). */
...
/** No row about comments or reports in this panel (the Moderator's control read). */
...
test.describe('Reader comments & moderation (U14)', () => {
    test('S1: switching public comments on and off', {tag: ['@smoke']}, async ({asUser, opsApi}, testInfo) => {
        ...
        await createServer(opsApi, tag, people);
        const {submissionId} = await postPreprint(opsApi, tag, people, {
            title,
            userComments: [{user: reader.username, text, approved: true}],
        });
        // The fresh server of the control: no `enablePublicComments`, no comments.
        const freshTag = `${tag}b`;
        const freshPeople = cast(freshTag);
        await createServer(opsApi, freshTag, freshPeople, {comments: false});
        ...
        // The side menu after the reload: on a preprint server the "Content"
        // group is gone altogether (beside the "Statistics" group it keeps).
        await openDashboard(page, tag);
        await expect(menu.groupHeader('Statistics')).toHaveCount(1);
        await expect(menu.groupHeader('Content')).toHaveCount(0);
        expect(await menu.contentEntries()).toEqual([]);
        ...
        // Switching on again: the same reload; the "Content" group is back
        // with "Comments" as its only entry.
        await settings.goto();
        await expect(settings.box()).not.toBeChecked();
        ...
        await expect(settings.box()).toBeChecked();
        await openDashboard(page, tag);
        await expect(menu.groupHeader('Content')).toHaveCount(1);
        expect(await menu.contentEntries()).toEqual(['Comments']);
        ...
        // Control: the fresh server's tab shows the box unticked, its side
        // menu offers no Content › Comments (no "Content" group at all,
        // beside "Statistics"), and its Comments page, opened by address,
        // reads "No Items" under every tab.
        ...
        const freshMenu = new EditorialSideMenu(freshPage);
        await expect(freshMenu.groupHeader('Statistics')).toHaveCount(1);
        await expect(freshMenu.groupHeader('Content')).toHaveCount(0);
        expect(await freshMenu.contentEntries()).toEqual([]);
        ...
        // The submission exists on the first server: the row read above was a real one.
        expect(submissionId).toBeGreaterThan(0);
    });

    test('S2: approving and hiding on the Comments page', async ({asUser, opsApi}, testInfo) => {
        test.setTimeout(300_000);
        ...
        await createServer(opsApi, tag, people);
        // The second server on the site, with a comment of its own.
        const otherTag = `${tag}b`;
        const otherPeople = cast(otherTag, {readers: 1});
        const otherText = `The other server's comment ${otherTag}.`;
        await createServer(opsApi, otherTag, otherPeople);
        const [{submissionId, userComments}] = await Promise.all([
            postPreprint(opsApi, tag, people, {
                title,
                userComments: [
            ...
            postPreprint(opsApi, otherTag, otherPeople, {
                title: `Other preprint ${otherTag}`,
        ...
        // Control: the second server's Comments page lists its own comment alone.
    ...
    test('S3: reports on the Comments page', async ({asUser, opsApi}, testInfo) => {
        ...
        await createServer(opsApi, tag, people);
        const {userComments} = await postPreprint(opsApi, tag, people, {
            title,
            userComments: [
                {
    ...
    test('S4: deleting a comment as a moderator', async ({asUser, opsApi}, testInfo) => {
        test.setTimeout(240_000);
    ...
    test("S5: the moderators' tasks", async ({asUser, opsApi, pkpMail}, testInfo) => {
        test.setTimeout(480_000);
        const tag = makeTag('s5', testInfo);
        const people = cast(tag, {readers: 2, sectionEditor: true});
        ...
        // The Preprint Server Manager's Tasks panel: the two unread rows
        // with the text and the reason under them, cut to 200 characters
        // and "..." (no Editor level on a preprint server: {OJS OMP}).
        ...
        await tasks.close();
        // Control: the Moderator's panel holds neither row.
        ...
        // After "Hide Comment": the "pending review" row stays in the
        ...
        // After "Delete Report": the "requires review" row leaves the
        ...
        // After "Delete Comment": the "pending review" row leaves the moderator's panel.
        await comments.goto(`?commentId=${commentId}`);
        await expect(comments.commentPanel()).toBeVisible({timeout: 30_000});
        await comments.deleteButton().click();
        await expect(comments.confirmDialog('Delete Comment')).toBeVisible();
        ...
        // Mailboxes: the writer's, the reporter's and the moderator's hold
        ...
        for (const person of [writer, reporter, people.manager]) {
    ...
    test('S6: who opens the Comments page', async ({asUser, opsApi, browser, baseURL}, testInfo) => {
        test.setTimeout(240_000);
        ...
        // The Moderator: no "Content" group (beside the "Statistics" group
        // it does show); the address answers the access-denied page.
        const sePage = await (await asUser(people.sectionEditor.username)).newPage();
        ...
        // Control: the Preprint Server Manager's side menu holds Content ›
        // Comments, and the address opens the page headed "Comments".
        const managerPage = await (await asUser(people.manager.username)).newPage();
    ...
    test('S7: more than a page of comments', async ({asUser, opsApi}, testInfo) => {
        test.setTimeout(240_000);
        ...
        await createServer(opsApi, tag, people);
        await postPreprint(opsApi, tag, people, {
            title,
    ...
    test('S14: no reader-side half on a preprint server (absence)', async ({asUser, opsApi, browser, baseURL}, testInfo) => {
        test.setTimeout(240_000);
        ...
        const title = `Preprint ${tag}`;
        await createServer(opsApi, tag, people);
        const {submissionId} = await postPreprint(opsApi, tag, people, {title});

        // The landing page, signed out: the preprint's page with none of
        // the blocks, beside its own headings and the header's "Login".
        const anon = await anonContext(browser, baseURL);
        const anonPage = await anon.newPage();
        const book = new PreprintLandingPage(anonPage, tag);
        await book.goto(submissionId, title);
        await expect(book.sectionHeading('Abstract')).toBeVisible();
        await expect(book.sectionHeading('Posted')).toBeVisible();
        ...
        const readerPage = await (await asUser(reader.username)).newPage();
        const readerBook = new PreprintLandingPage(readerPage, tag);
        await readerBook.goto(submissionId, title);
        await expect(readerBook.signedInName(reader.username)).toBeVisible();
        await expect(readerBook.sectionHeading('Abstract')).toBeVisible();
        ...
        // The Comments page: "Content" holds "Comments" alone; pressed, the
        // page headed "Comments" lists "No Items" under each tab.
        await openDashboard(page, tag);
        const menu = new EditorialSideMenu(page);
        await expect(menu.groupHeader('Content')).toHaveCount(1);
        expect(await menu.contentEntries()).toEqual(['Comments']);
        ...
        // Control: untick and "Save" (scenario 1): the "Content" group
        // leaves the side menu (the "Statistics" group stays) while the
        // landing page reads exactly as before.
        await settings.goto();
        await settings.box().uncheck();
        await settings.save();
        await settings.openCommentsTab();
