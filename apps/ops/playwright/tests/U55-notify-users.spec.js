 ...
 *
 * Notify users (bulk email) — OPS suite, the parallel part: scenario 3 (the
 * Site Administrator takes a role off the "Notify" tab), which sends
 * nothing. Scenarios 1, 2, 4 and 5 wait for the site's background jobs and
 * live in ./serial/U55-notify-users.spec.js. In OPS vocabulary: a
 * preprint server, its Preprint Server Manager, "Hosted Servers", the
 * wizard's first tab "Server Settings", the Settings entry "Server".
 ...
 * scenarios/context` with `bulkEmails: true` (the server ticked under
 * "Bulk Emails") and throwaway accounts; the Site Administrator is `admin`,
 * whom the context factory enrols as a manager of every scratch server.
 * "Author" is ticked under "Disable Roles" on the screen, as the scenario
 * does; the change is the server's own, so it runs beside other workers.
 * No test saves Administration › Site Settings › "Bulk Emails" here (its
 * "Save" posts the list as its page loaded it). Every actor is opened
 * through `asUser` (no default user: a multi-actor test sets none,
 * patterns.md "Fixture selection").
 */
...
const DISABLE_DESCRIPTION =
    'A server manager will be unable to send bulk emails to any of the roles selected below. Use this setting to ' +
    ...
    'can be disabled completely for this server in Admin > Site Settings.';
...
test.describe('notify users', () => {
    test('S3: the Site Administrator takes a role off the "Notify" tab', async ({asUser, opsApi}, testInfo) => {
        test.slow();
        ...
        // The server's roles as its "Roles" tab lists them (the order the
        // side tab follows).
        ...
        // The Preprint Server Manager: "Author" not offered, every other
        // role is, and nothing says a role is withheld; the "Roles" tab
        // keeps "Author" (Rules 3, 12).
