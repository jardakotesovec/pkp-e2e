 ...
 * canonical scenario the spec runs on a preprint server (scenarios 1–8;
 * scenario 9, the issue email, is OJS-only: a preprint server has no issues
 * and no issue row, and S7's exact row list is where that absence is
 * asserted, with the rows that are listed as the control). Read the spec's
 * journal words as the title note says: "server" for "journal", Preprint
 * Server Manager for Journal Manager, Moderator for Section Editor, and no
 * reviewer role at all. The "needs an editor" task reads "A new preprint
 * has been submitted to which a moderator needs to be assigned." here, and
 * the workflow's only stage is Production, so scenario 5's discussion is
 * opened in the panel headed "Production Tasks & Discussions".
 ...
 * rule):
 * - OPS3 🐞 (pressing a task lands on "A workflow stage was not specified."
 *   instead of the preprint): S1 presses the row and asserts what Rule 2c
 *   promises either way — the task is read and the bell drops — and never
 *   where the browser lands.
 * - OPS2 🐞 (the stray space in "A new preprint , "Title", has been
 *   submitted."): S7 compares the rows with whitespace before punctuation
 *   removed, so the row's words are asserted and its spacing neither way.
 * - OPS1 ❓ (the reviewer row and the "Weekly email" row on a server that
 *   never raises them): S7 asserts them as listed, which is what Fields
 *   says; nothing asserts that they are ever raised.
 ...
 * - A3 🐞 (no reader-side count for a Moderator) and Rule 4: the
 ...
 * - A9 ❓ (the Unsubscribe page lists the statistics box on a server whose
 *   email is off): every server here keeps the statistics email on.
 * - Rule 2b's server initials on a multi-server account and its paging
 *   beyond 25 rows; Rule 2d's site-level bell; Rule 5d's site-level choices;
 *   Rule 6's other rows (announcements, versions, the scheduled emails);
 ...
 *
 * Isolation: every test seeds its own scratch preprint server with throwaway
 ...
 * row is written here without OPS2's stray space; `tidyRows()` removes any
...
/** The Public Announcements rows' setting names (Rule 5e, note s8): one row on a server. */
const PUBLIC_SETTINGS = ['notificationNewAnnouncement'];

/** The Unsubscribe page's boxes on a server: the tab's rows without the two issue boxes (Fields). */
const UNSUBSCRIBE_BOX_COUNT = 9;
...
/** The tab as read, with whitespace before punctuation removed (OPS2 is not the contract). */
function tidyRows(table) {
    return table.map(({group, rows}) => ({group, rows: rows.map((row) => row.replace(/\s+([,.])/g, '$1'))}));
}
...
test.describe('notifications center & email preferences', () => {
    ...
    test('S2: Mark New, Mark Read, Delete', async ({asUser, opsApi}, testInfo) => {
        ...
        for (const title of titles) {
            await opsApi.createSubmission({tag, context: server.path, submitter: author, title});
        }
    ...
    test('S3: unticking "Enable…" stops the task', async ({asUser, opsApi, pkpMail}, testInfo) => {
        test.slow();
    ...
    test("S5: unsubscribing from an email's footer link", async ({browser, baseURL, asUser, opsApi, pkpMail}, testInfo) => {
        ...
        expect(authorMail.full.Text).toMatch(
            new RegExp(`Reply to this comment at #${submissionId} Authorson.*or unsubscribe.*from emails sent by ${server.name}`, 's')
        );
        ...
        try {
            ...
            expect(boxCount).toBe(UNSUBSCRIBE_BOX_COUNT);
    ...
    test('S6: the link acts on the addressee, not on whoever is signed in', async ({asUser, opsApi, pkpMail}, testInfo) => {
        test.slow();
        test.setTimeout(180_000);
        ...
        const managerProfile = await openNotificationsTab(managerPage, server.path);
    ...
    test('S7: the rows, per application, and the site-level tab', async ({asUser, opsApi}, testInfo) => {
        test.slow();
        // A scratch server, so the site holds more than one server (Rule 5d).
        const tag = makeTag('s7', testInfo);
        await seedServer(opsApi, tag, [{username: `${tag}mg`, roles: ['manager']}]);

        // The Preprint Server Manager on the seeded server: the four groups
        // with exactly the OPS rows (no issue rows: a server has no issues),
