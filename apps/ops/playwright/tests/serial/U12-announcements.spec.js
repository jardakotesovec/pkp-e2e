 ...
 * OPS suite: S4 (the email and who gets it) drains the site's job queue
 * with `runJobs()`, which pops the SHARED queue and must never run while
 * parallel workers seed (patterns.md parallel lesson 7); S6 (the site's
 * announcements) ticks the site's own "Enable announcements", a setting
 * every test reading the site's home page would see, and restores it
 * (PRINCIPLES A7, A9). On a preprint server the roles are the Preprint
 * Server Manager, the Moderator (the `sectionEditor` key), the Author and
 * the Reader; the Notifications row reads "A new announcement has been
 * created." here as on a journal.
 ...
 * - S6's Tasks silence is not re-read (S4 reads it once on a server);
 * - the site's announcements and types are shared state across runs, so
 *   S6 removes the type it added before unticking and sweeps its rows in
 *   a `finally` (PRINCIPLES A7).
 *
 * S6 carries `@solo` and runs alone in the `ops-solo` project: its notices
...
test.describe('announcements (queued email; the site)', () => {
    test('S4: the email and who gets it', async ({browser, baseURL, asUser, opsApi, pkpMail}, testInfo) => {
        test.slow();
        ...
        // row, the Moderator ticks "Do not send me an email…".
        ...
        const sePage = await (await asUser(moderator)).newPage();
        ...
        // The row's defaults, as the Preprint Server Manager: "Enable…"
        // ticked and "Do not send me an email…" unticked on "A new
        // announcement has been created." (Settings).
        ...
        // holds one "New Announcement" email for the manager and one for
        // the Reader, subject the title, From the manager, the body with
        // the title in bold, the short description, the sentence with its
        ...
        for (const [who, summary] of [
            ...
            expect(summary.From.Name, `${who}'s From line`).toBe('Mona Manager');
        ...
        // Who gets nothing: no email for the Author or the Moderator, none
        // titled "Board meeting" for anyone; the Reader's email above,
        ...
        expect(await pkpMail.count({to: mailOf(moderator), contains: tag}), 'no mail to the moderator').toBe(0);
        for (const username of [manager, reader, author, moderator]) {
        ...
        // the jobs ran; then a preprint seeded on the server is the
        ...
        await tasks.close();
        await opsApi.createSubmission({tag, context: tag, submitter: spare, title: `Submission ${tag}`});
        await page.reload();
        await tasks.expectCount(1);

        // Control: the seeded server's Reader, with no role in the scratch
        // server, gets no email either, read after the Reader's arrived
        // (Actors row 6; Rule 1).
        expect(await pkpMail.count({to: 'reader.rosa@mail.test', contains: tag}), 'no mail to reader.rosa').toBe(0);
    });

    test("S6: the site's announcements @solo", async ({browser, baseURL, asUser, opsApi, pkpMail}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        try {
            ...
            // page; the scratch server's pages show none of it (Rules 1,
            // 9–11, 16).
            ...
            const serverPublic = new PublicAnnouncements(visitor, tag);
            await serverPublic.goto();
            await expect(serverPublic.articles()).toHaveCount(0);
            await expect(serverPublic.container()).not.toContainText('Site maintenance');
            ...
            // Control first, so the silence below rides on it: "Server
            // notice", added on the scratch server with the email box
            // ticked, mails the Site Administrator, who holds a role there
            // (Actors row 6).
            const managerPage = await (await asUser(manager)).newPage();
            const serverAdmin = new AnnouncementsPage(managerPage, tag);
            await serverAdmin.goto();
            await addAnnouncement(serverAdmin.list(), {title: `Server notice ${tag}`, shortDescription: 'From the server.', sendEmail: true});
            await managerPage.close();

            // No email: the site's background jobs run; the mail catcher
            // holds no email titled "Site maintenance" for the Site
            // Administrator, read after the server's notice arrived
            // (Actors row 6; Rule 16).
            runJobs();
            await pkpMail.expectNone({
                to: ADMIN_MAIL,
                subject: 'Site maintenance',
                afterControl: {to: ADMIN_MAIL, subject: `Server notice ${tag}`},
            });
