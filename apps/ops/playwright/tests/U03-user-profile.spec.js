 ...
 * a preprint server (scenarios 1–10 in their OPS form; the spec lists no
 * OPS-specific scenario). Read the spec's journal words as the title note
 * says: "server" for "journal", Preprint Server Manager for Journal Manager,
 * Moderator for Section Editor, and no reviewer role at all.
 * Spec: docs/specs/U03-user-profile.md
 *
 * OPS form of the scenarios (register OPS1 ✅, an intended difference):
 * - Scenario 6 names a "Reviewer" box and a "Reviewing interests" tag box.
 *   A preprint server offers "Reader" and "Author" only and shows no
 *   interests box, so S6 runs the absence form (RUNBOOK multi-app rule 3,
 *   PRINCIPLES M4): no reviewer box and no interests box, each paired with
 *   a positive control taken the same way (the two boxes that are offered,
 *   in the same section; the fold that ends the tab). The fold reads
 *   "Register with other servers" / "Hide other servers".
 * - Scenario 10 runs as a scratch server's own manager (OPS seeds no
 *   editor; a Preprint Server Manager is the role the spec names).
 *
 ...
 *   one-server user): every test opens the server-level page; the
 *   site-level profile of Rule 3 is not driven.
 ...
 * - A4 🐞 (a closed server keeps an empty section on the Roles tab): the
 *   registration setting is not touched.
 * - A5 ❓ (the scheduled-role banner ignores which server is open) and Rule
 *   13: a dated role invitation is the user-invitations feature's surface.
 * - A6 ❓ (a self-service box also drops a manager-granted role, locking an
 *   author out of My Submissions, seen on OPS too): S6 ticks and unticks a
 *   role the user took themselves and never asserts the lock-out.
 ...
 *   page is opened; seed-facts also says the picture never renders on the
 *   test installs, so S7 asserts the "Delete" button, never the picture.
 ...
 * - OPS2 🐞 (the email-change template is missing from a preprint server's
 *   Manage Emails list): S4 asserts the message that is sent, which is the
 *   contract; the emails list is Emails management's surface and the
 *   missing row is never asserted either way.
 ...
 *   (a site-wide change) and the masthead affiliation; Rule 8d {OJS OMP}
 *   (interests and the Add Reviewer search: no such surface on OPS beyond
 *   S6's absence assertion); Rule 10a's compromised-password refusal (a
 *   site-settings singleton, PRINCIPLES A7/A9); Rule 12d (no API secret: a
 *   config state with no screen, PRINCIPLES D9); the audit lines and the
 ...
 * server seeded through the scenario endpoint (unique tags; the scenario
 * API rejects reviewer roles on OPS, and none is asked for); the roster and
 * `publicknowledge` are only read (S1). Every sign-in happens in the test's
 * own fresh browser context through the real Login form, never through the
 * shared .auth cache, because a password change ends the account's other
 * sessions and a sign-out ends the cached one; the only cached identity
 * used is `admin` (a scratch server's auto-enrolled manager) for the Users
 * & Roles reads. Mailpit reads are scoped by the throwaway recipient
 * (PRINCIPLES A8) and every silence claim has a positive control. A refused
 * image upload raises a browser alert (Rule 9a), so S7 registers a dialog
 * handler before its upload. No hard-coded waits.
 */
...
/** Users & Roles (Users tab) of a scratch server, as its auto-enrolled manager `admin`. */
async function usersRolesRow(adminPage, contextPath, username, email) {
    const usersRoles = new UsersRolesPage(adminPage, contextPath);
...
test.describe('user profile', () => {
    test('S1: reach the profile and its tabs', {tag: '@smoke'}, async ({browser, baseURL}) => {
        ...
        try {
            ...
            const profile = new ProfilePage(page, SERVER);
    ...
    test('S2: rename yourself and change your initials', async ({browser, baseURL, opsApi, asUser}, testInfo) => {
        test.slow();
        ...
        try {
            ...
            // name (Rule 4; `admin` is the scratch server's auto-enrolled
            // manager).
    ...
    test('S3: update contact details', async ({browser, baseURL, opsApi}, testInfo) => {
        ...
        try {
            ...
            // Another account's address (a ready account of the roster):
    ...
    test('S6: take a role and give it up (absence form: no reviewer box, no interests box)', async ({browser, baseURL, opsApi, asUser}, testInfo) => {
        ...
        try {
            ...
            expect(await profile.currentContextRoleLabels()).toEqual(['Reader', 'Author']);
            ...
            // is there, and open it reads "Hide other servers".
            ...
            await profile.otherContextsLink().click();
    ...
    test('S8: change the password', async ({browser, baseURL, opsApi}, testInfo) => {
        test.slow();
        test.setTimeout(180_000);
