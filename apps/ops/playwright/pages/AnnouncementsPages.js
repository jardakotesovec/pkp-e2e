 ...
 *
 * OPS-local Page Objects and flow helpers for the Announcements feature
 * (spec: docs/specs/U12-announcements.md), as a preprint server shows it:
 * the settings sentence names "server news and events", the header's
 * "Announcements" item comes first, the stored picture lives under
 * `public/contexts/<id>/announcements/`, and no feed plugin is installed.
 *
 * Surfaces:
 ...
 * - AnnouncementsPage — the server's Announcements page
 ...
 * - PublicAnnouncements / AnnouncementView / homeBlock / primaryNavItems —
 *   the reader's side: the Announcements page, an announcement's page, the
 *   home page block and skip link, the header's primary navigation.
 * - pluginRows / sidebarField / feedUrl — the S7 absence: the Plugins grid's
 *   rows, the Appearance › Setup "Sidebar" list and the feed addresses a
 *   journal's plugin would answer.
 *
 ...
 * 2026-09-17), by the OJS and OMP suites, and while this suite was built.
 */
...
/**
 * The three-field settings form, shared by the server's tab and the site's
...
class AnnouncementsSettingsForm {
    ...
    introductionField(locale = 'en') {
        return this.panel
            ...
            .filter({has: this.page.locator(`iframe[id*="announcementsIntroduction"][id$="-${locale}_ifr"]`)});
    }
...
exports.AnnouncementsSettingsTab = class AnnouncementsSettingsTab extends AnnouncementsSettingsForm {
    /**
     * @param {import('@playwright/test').Page} page
     * @param {string} contextPath the server's url path
     */
...
exports.SiteAnnouncementsTab = class SiteAnnouncementsTab {
    ...
    /** The site's types grid on the "Announcement Types" side tab (three nested matches: the first). */
    types() {
...
/** A list panel of announcements: the server's tab or the site's side tab. */
...
/** The "Announcement Types" legacy grid: the server's tab or the site's side tab. */
class AnnouncementTypesGrid {
    ...
    row(name) {
        return this.rows().filter({hasText: new RegExp(`(^|\\s)${escapeRe(name)}\\s*$`)});
    }
...
const AnnouncementPanel = class AnnouncementPanel {
    ...
    /** The "Announcement Type" field (present only while the server has a type). */
    typeField() {
...
/** The header's item of the given name ("Announcements"). */
exports.primaryNavItem = function primaryNavItem(page, name) {
    return page.locator('#navigationPrimary > li > a').filter({hasText: new RegExp(`^\\s*${escapeRe(name)}\\s*$`)});
};
...
exports.PublicAnnouncements = class PublicAnnouncements {
    /**
     * @param {import('@playwright/test').Page} page
     * @param {string} contextPath the server's url path ('index' for the site's page)
     */
    constructor(page, contextPath) {
        this.page = page;
        this.contextPath = contextPath;
    ...
    summary(title) {
        return summaryParts(
            this.articles().filter({has: this.page.locator('h2 a', {hasText: new RegExp(`^\\s*${escapeRe(title)}\\s*$`)})})
        );
    }
};
...
/** The server's (or the site's) home page in a page, in the given language ('' for the bare address). */
...
// -------------------------------------------------------------------------
// The S7 absence: no announcement feed plugin on a preprint server (Rule 18)
// -------------------------------------------------------------------------

/**
 * Open Settings › Website › Plugins and return the grid's rows
 * (`#pluginGridContainer tr.gridRow`), on screen once the grid has loaded.
 */
exports.pluginRows = async function pluginRows(page, contextPath) {
    await page.goto(`/index.php/${contextPath}/en/management/settings/website`);
...
/**
 * Open Settings › Website › Appearance › Setup and return the "Sidebar"
 * field (the list of block checkboxes).
 */
exports.sidebarField = async function sidebarField(page, contextPath) {
    await page.goto(`/index.php/${contextPath}/en/management/settings/website`);
    const tab = page.locator('#appearance-button').first();
    await expect(tab).toBeVisible({timeout: T});
    if ((await tab.getAttribute('aria-selected')) !== 'true') {
        await tab.click();
    }
    const appearance = page.locator('#appearance').first();
    await appearance.getByRole('tab', {name: 'Setup', exact: true}).click();
    const field = appearance.locator('[role="tabpanel"]:visible .pkpFormField').filter({hasText: 'Sidebar'}).first();
    await expect(field).toBeVisible({timeout: T});
    return field;
};

/** A feed's address as a journal's plugin would answer it (`gateway/plugin/AnnouncementFeedGatewayPlugin/{atom|rss2|rss}`). */
