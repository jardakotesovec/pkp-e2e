 ...
 *
 * Reader comments & moderation — the OPS page objects (feature spec:
 * docs/specs/U14-reader-comments-and-moderation.md). A preprint server installs the
 ...
 * (Rules 10 to 16). The preprint's landing page carries no comments block
 * (the absence paragraph), which `PreprintLandingPage` reads.
 *
 * DOM facts the locators rely on (U14 claim checks K1, K3 and K4, all three
 * apps, 2026-09-16, `.reports/U14/screen-notes.md`; the OPS rows of
 ...
 *   open header carries `aria-expanded` (patterns.md pitfall 2). On a
 *   preprint server the "Content" group exists only while public
 *   comments are on and then holds "Comments" alone (Rule 3c), so
 *   `EditorialSideMenu.contentEntries()` reads `[]` with the setting off
 *   and `groupHeader('Content')` resolves to nothing.
...
exports.CommentsSettingsTab = class CommentsSettingsTab extends BasePage {
     ...
     * @param {string} contextPath the preprint server's path
     */
...
/**
 * The preprint's landing page (`preprint/view/{id}`): what a preprint
 * server's landing page does NOT carry (the absence paragraph), read
 * beside the page's own content as the control. The OPS frontend keeps
 * a `main` landmark (an `article` with the title as its level-1 heading
 * and "Authors", "Abstract", "Posted", "Versions" as level-2 headings);
 * the header's user menu is `#navigationUserWrapper` (K1's OPS landing
 * snapshots, `.reports/U14/ccK1/on-anon-landing-ops.json`).
 */
exports.PreprintLandingPage = class PreprintLandingPage extends BasePage {
    /**
     * @param {import('@playwright/test').Page} page
     * @param {string} contextPath the preprint server's path (a scratch server is single-locale: bare address)
     */
    constructor(page, contextPath) {
        super(page);
        this.contextPath = contextPath;
    ...
    url(submissionId) {
        return this.contextUrl(this.contextPath, `/preprint/view/${submissionId}`);
    }
    ...
    /** One of the page's own level-2 headings ("Abstract", "Posted", …): the absence read's positive control. */
    sectionHeading(name) {
     ...
     * read): the body, so the header's signed-in / signed-out state and
     * the article are read in one string.
     */
    async mainText() {
