 ...
 *
 * Roles configuration — OMP suite, one test per canonical scenario the spec
 * runs on a press (scenarios 1–7, common, in OMP vocabulary: press, Press
 * Manager, Series Editor, the five stage columns "Submission", "Internal
 * Review", "External Review", "Copyediting", "Production"; scenario 8, the
 * press's open access files, OMP only).
 ...
 *
 * Every test seeds its own scratch press with throwaway accounts (footnote
 ...
 * "Fixture selection"); the signed-out visitor of S7 and S8 is a fresh
 ...
 * beside a positive control taken the same way (M4, M6). Everything runs in
 * the parallel `omp` project.
 */
...
const S = 'Submission';
const IR = 'Internal Review';
const ER = 'External Review';
const C = 'Copyediting';
const P = 'Production';
const STAGES = [S, IR, ER, C, P];
const COLUMNS = ['Role Name', 'Permission level', ...STAGES];
const LEVEL = {
    manager: 'Press Manager',
    subEditor: 'Series Editor',
    assistant: 'Assistant',
    author: 'Author',
    reviewer: 'Reviewer',
    reader: 'Reader',
...
/**
 * The roles of a new press, in the list's order (the spec's roles table and
 * the press paragraph under it): name, level, the stages ticked, and the
 * open boxes of the row (the rest greyed).
 */
const DEFAULT_ROLES = [
    {name: 'Press manager', level: LEVEL.manager, stages: null, open: []},
    {name: 'Press editor', level: LEVEL.manager, stages: STAGES, open: []},
    {name: 'Production editor', level: LEVEL.manager, stages: [C, P], open: []},
    {name: 'Series editor', level: LEVEL.subEditor, stages: STAGES, open: STAGES},
    {name: 'Copyeditor', level: LEVEL.assistant, stages: [C], open: STAGES},
    {name: 'Designer', level: LEVEL.assistant, stages: [P], open: STAGES},
    {name: 'Funding coordinator', level: LEVEL.assistant, stages: [S, IR, ER], open: STAGES},
    ...
    {name: 'Author', level: LEVEL.author, stages: STAGES, open: STAGES},
    {name: 'Volume editor', level: LEVEL.author, stages: STAGES, open: STAGES},
    {name: 'Chapter Author', level: LEVEL.author, stages: [C, P], open: STAGES},
    {name: 'Translator', level: LEVEL.author, stages: STAGES, open: STAGES},
    {name: 'Internal Reviewer', level: LEVEL.reviewer, stages: [IR], open: [IR, ER]},
    {name: 'External Reviewer', level: LEVEL.reviewer, stages: [ER], open: [IR, ER]},
...
const SITE = {
    ...
    siteBox: 'Users must be registered and log in to view the press site.',
    contentGroup: 'View Monograph Content',
    contentBox: 'Users must be registered and log in to view open access content.',
    registrationGroup: 'User Registration',
    open: 'Visitors can register a user account with the press.',
    closed:
        'The Press Manager will register all user accounts. Editors or Section Editors may register user accounts for reviewers.',
    closedMessage: 'This press is currently not accepting user registrations.',
};

/** Unique per-run tag: single alphanumeric token, app + scenario + worker. */
function makeTag(scenario, testInfo) {
    return `u54s${scenario}ompw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
/** The front end's user navigation ("Register", "Login"). */
...
async function inviteRoleOptions(page, contextPath, email) {
    ...
    await expect(wizard.searchField).toBeVisible({timeout: 30_000});
    await wizard.searchFor(email);
    await expect(page.getByRole('heading', {name: /Enter details/})).toBeVisible({timeout: 30_000});
    if ((await wizard.newRoleRows().count()) === 0) {
        await wizard.addRoleButton.click();
    }
    const select = wizard.newRoleRows().first().locator('select[name="userGroupId"]');
...
test.describe('roles configuration', () => {
    test('S1: the "Roles" list of a new press', async ({asUser, ompApi}, testInfo) => {
        test.slow();
        ...
        // The rows: the nineteen roles of a press, in no fixed order (A13),
        // each with its level and, but for the manager row (A2), its stages;
        // the line (Rules 2, 4).
        ...
        await roles.expectPagingLine('1 - 19 of 19 items');
        ...
        await expect(roles.rows()).toHaveCount(4);
        expect(unordered(await roles.rowNames())).toEqual(unordered(['Author', 'Volume editor', 'Chapter Author', 'Translator']));
        await roles.expectPagingLine('1 - 4 of 4 items');
        ...
        // Ten per page: ten rows, then the other nine; which roles each
        ...
        await roles.expectPagingLine('1 - 10 of 19 items');
        await expect(roles.pageLink('2')).toBeVisible();
        await roles.gotoListPage('2');
        await expect(roles.rows()).toHaveCount(9);
        await roles.expectPagingLine('11 - 19 of 19 items');
        await roles.reload();
        await expect(roles.rows()).toHaveCount(DEFAULT_ROLES.length);
        await roles.expectPagingLine('1 - 19 of 19 items');
    ...
    test('S2: give a role a stage, then take it away', async ({asUser, ompApi}, testInfo) => {
        test.slow();
    ...
    test('S3: create a role', async ({asUser, ompApi}, testInfo) => {
        ...
        /** The three level-bound boxes, the metadata box and the masthead box, as the table gives them. */
        ...
        // "Reviewer": only "Internal Review" and "External Review" open.
        await win.chooseLevel(LEVEL.reviewer);
        await expectStages([IR, ER]);
        await expectOptions({selfReg: true, recommend: false, settings: false});
    ...
    test('S4: rename a role', async ({asUser, ompApi}, testInfo) => {
        ...
        // T-omp-1, test-omp-findings.md; T-ojs-1 on OJS).
        await mp.reload();
        await roles.usersTab.click();
    ...
    test('S5: the Settings box of a manager\'s only Settings role', async ({asUser, ompApi}, testInfo) => {
        ...
        // "Managing editor" for the Press Manager (Rules 12, 14, 15a, 18).
        const mp = await signedIn(asUser, manager.username);
        const roles = new RolesTab(mp, tag, {stages: STAGES});
        await roles.goto();
        ...
        // Control: the Press Manager's "Edit" of "Managing editor": ticked and open.
        await roles.goto();
    ...
    test('S6: remove a role', async ({asUser, ompApi}, testInfo) => {
        ...
        // A role the press was created with (Rule 21).
    ...
    test('S7: require sign-in, then close registration', async ({asUser, ompApi, browser, baseURL}, testInfo) => {
        ...
        // The signed-out visitor, on the press's home page.
        const visitor = await anonPage(browser, baseURL);
    ...
    test("S8: sign-in for a press's open access files", async ({asUser, ompApi, browser, baseURL}, testInfo) => {
        test.slow();
        ...
        await ompApi.createContext({
            tag,
            users: [manager, {username: author, givenName: 'Ada', familyName: 'Author', roles: ['author']}],
        });
        ...
        const bookUrl = `/index.php/${tag}/catalog/book/${submissionId}`;

        // The signed-out visitor, on the book page, which lists "PDF".
        const visitor = await anonPage(browser, baseURL);
        const vp = visitor.page;
        await vp.goto(bookUrl);
        const bookTitle = vp.locator('.pkp_structure_main h1');
        const pdfLink = vp.locator(`a[href*="/catalog/view/${submissionId}/"]`).filter({hasText: 'PDF'});
        await expect(bookTitle).toHaveText(TITLE);
        await expect(pdfLink).toHaveCount(1);

        // The box ticked (Rule 23).
        const mp = await signedIn(asUser, manager.username);
        const access = new SiteAccessTab(mp, tag);
        await access.goto();
        const contentBox = access.group(SITE.contentGroup).getByRole('checkbox', {name: SITE.contentBox, exact: true});
        await expect(contentBox).not.toBeChecked();
        await contentBox.check();
        expect((await access.save()).status()).toBe(200);

        // Control: the book page itself opens for the visitor with the box
        // ticked, no Login on it (Rule 24).
        await vp.reload();
        await expect(bookTitle).toHaveText(TITLE);
        expect(new URL(vp.url()).pathname).toBe(bookUrl);
        await expect(vp.locator('form#login')).toHaveCount(0);
        await expect(pdfLink).toHaveCount(1);

        // The visitor presses "PDF": the Login page shows (Rule 24).
        await pdfLink.click();
        await expect(vp.locator('form#login')).toBeVisible();
        expect(new URL(vp.url()).pathname).toBe(`/index.php/${tag}/login`);
        await visitor.context.close();
    });
});
