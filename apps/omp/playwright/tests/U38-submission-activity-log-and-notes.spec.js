 ...
 *
 * Submission activity log & notes — OMP suite, one test per canonical
 * scenario the spec runs on a press (the common scenarios 1 and 2 and the
 * journal-and-press scenarios 3 and 4), in the press's own words: the
 * submit line "Initial submission completed.", the Series editor as the
 * assigned sub-editor, the "External Review" round.
 ...
 * seeded press, in series `monographs`, as the Press manager
 * `manager.maya`. S1 runs on a scratch press with throwaway accounts (a
 * manager, a Series editor assigned through `participants[]`, an Author):
 * it reads "Note posted." and "Note deleted.", which the app queues per
 * user and the next page fetch of ANY session of that user takes
 * (patterns.md parallel lesson 2), so a roster persona used by other
 * workers could lose them. The Site Administrator of S1 and the manager of
 * S4's control is `admin`, a manager of every scratch press (s0). S4 builds
 * its two presses as footnote s0 says. Lines written in the same second
 * have no fixed order (Rule 2), so a line or a note is found by its text,
 * never by its position; the one positional read is a "top line" a
 * scenario names, a note posted seconds after everything else. Every
 * absence is read settled (the tab's own fetch answered, an exact list
 * compared) and paired with a positive control taken the same way (M4,
 * M6). The window's close question is a browser dialog, and a page that
 * dropped a note can raise the leave-page question: every page records its
 * browser dialogs from the start and accepts any the test did not script.
 * Downloads are read through Playwright's download event (the suggested
 * name). Waits are web-first (A5). Everything runs in the parallel `omp`
 * project.
 */
...
const {loginAsDialog, confirmLoginAsDialog, userNav} = require('../pages/LoginSessionsPages.js');
...
/** The press's submit line (Rule 3; lib/pkp's wording, which OMP keeps). */
const SUBMIT_LINE = 'Initial submission completed.';
...
function makeTag(scenario, testInfo) {
    return `u38${scenario}ompw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}

/**
 * Seed a monograph: on the seeded press in series `monographs` (s0); on a
 * scratch press (no series) as it comes.
 */
...
async function seedPress(ompApi, tag, accounts, extra = {}) {
    ...
    await ompApi.createContext({tag, context: {name: `Press ${tag}`}, users, ...extra});
    return people;
}
 ...
 */
async function openWorkflow(asUser, appContext, username, submissionId, {contextPath = PRESS, menuKey = null} = {}) {
...
test.describe('submission activity log & notes', () => {
    test('S1: a note posted, read on "History" and deleted', {tag: '@smoke'}, async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        const tag = makeTag('s1', testInfo);
        const people = await seedPress(ompApi, tag, {
            mg: ['Mira', 'Manager', ['manager']],
            se: ['Sean', 'Series', ['sectionEditor']],
        ...
        // the press's submit line; every "Date" a day alone, newest first;
        ...
        // The Series editor's "Delete": the line and the note under the
        ...
        // The line kept: "History" holds the same lines as when the Series
        ...
        // Under "Login As": the administrator, acting as the Series editor
        // through the Participants row's "Login As", posts a note: listed
        // under the Series editor's name; its line reads "{administrator}
        // (acting as {Series editor})" (Rules 4b, 10).
        ...
        await expect(loginAsDialog(ad.page)).toBeVisible();
        await confirmLoginAsDialog(ad.page);
        // The page the sign-in lands on carries the acted-as username in the user menu.
        await expect(userNav(ad.page).locator('> button')).toContainText(people.se.username, {timeout: 30_000});
        ...
        // The manager's "Delete" on the Series editor's note: "Note
    ...
    test('S2: an email line and "View Email"', async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
    ...
    test('S3: file lines and "Download"', async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        const tag = makeTag('s3', testInfo);
        const {submissionId} = await seed(ompApi, tag, {files: [{file: 'article.pdf'}]});
        const {page, frame, log} = await openWorkflow(asUser, appContext, MANAGER, submissionId, {menuKey: MENU_KEYS.submission});
    ...
    test('S4: an editor who is also the author', async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        ...
        // Given (footnote s0): a user holding Author and Series editor who
        // submitted and is assigned as Series editor; one Reviewer accepted,
        // another declined, on the External Review round; on a press at the
        // install's review type ("Anonymous Reviewer/Anonymous Author") and on
        // one set to "Open".
        const build = async (pressTag, extra = {}) => {
            const people = await seedPress(
                ompApi,
                pressTag,
                {
            ...
            const seeded = await seed(ompApi, `${pressTag}s`, {
                context: pressTag,
            ...
            return {pressTag, people, submissionId: seeded.submissionId};
        };
        ...
        const reviewLines = async (username, press) => {
            const {log} = await openWorkflow(asUser, appContext, username, press.submissionId, {contextPath: press.pressTag});
            await log.open();
        ...
        const namesOf = (press) => [
            press.people.ra.name,
            press.people.rd.name,
            press.people.ra.username,
            press.people.rd.username,
        ];
        ...
        // Control: the press manager on the first press's submission: the
