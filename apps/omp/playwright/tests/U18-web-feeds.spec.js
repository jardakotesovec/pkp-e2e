 ...
 *
 * Web feeds — OMP suite, one test per canonical scenario: S1–S8, all common
 * to the three apps, in the press's own words (Press Manager, a book's
 * page at `catalog/book/{id}`, "Display web feed links on homepage
 * only.", a press's list always the most recently changed books up to the
 * number). The {OJS} bullets ("Future Tides" scheduled in an unpublished
 * issue, "…on issue pages only.", the current-issue choice, the "Archive"
 * and issue pages) have no press end and are not run here.
 * Spec: docs/specs/U18-web-feeds.md
 *
 * Deliberately NOT covered (register IDs from the spec's Findings register;
 * a 🐞 is never asserted as the contract, a ❓ is parked, not a gap; the
 * spec's Coverage section is the record of everything else left out):
 * - A1 🐞: S7 never reads the RSS 2.0 feed of the press with nothing
 *   published.
 * - A2 🐞: no unknown feed name is opened.
 * - A3 🐞, A7 🐞: S5's "Gamma" carries no keyword, subject, discipline or
 *   ISBN, and no ISBN line is looked for.
 * - A4 ❓: S4 reads the header's links, never the box, at each choice.
 * - A8 🐞: S2 reads which books the feeds list after the second publish
 *   and after the new version, never their order.
 * - A5 ❓, OPS1 🐞: a journal's and a preprint server's, in those suites.
 ...
 * press, its path the test's unique tag, with throwaway accounts (the
 * username twice as password): an Author who submits every book, a Press
 * Manager in S2–S7, a Reader in S8. Books are published by the submission
 * scenario (`published`, `datePublished`), one after another in the order
 * the recent list follows, a second apart on the server's clock (the last
 * change is stamped to the second); S5's categories come from the
 * context's `categories[]` and the book's `categories` (a scratch press
 * has no series). S3 alone passes `plugins` (the saved number 5) and no
 * test seeds `sidebar` (S3 places the box on screen); S7 names the press,
 * its initials and its principal contact; S8 passes `restrictSiteAccess`.
 * The scratch press has no Country and no key sets one, so S7 picks one on
 * the "Masthead" tab before that tab's "Save" (T-ojs-1). The empty-press
 * reads (S6, S7) use a new scratch press: publicknowledge collects the
 * suites' published books. Every signed-in actor gets its own `asUser`
 * context; the visitor is a browser context with an empty storage state
 * (patterns.md, parallel lesson 8). The feeds are read with the visitor's
 * request context and parsed (WebFeedPages.readFeed); the RSS 1.0 address
 * is also opened in the browser, which downloads "rss.rdf", and the Atom
 * and RSS 2.0 addresses show as text. Every Settings › Website ›
 * "Plugins" load fires the Plugin Gallery grid's server 500 (U62's
 * finding): not asserted here. Every absence is read settled and paired
 * with a positive control taken the same way (M4, M6): a feed's whole
 * list of titles beside the absent title, the header's feed links counted
 * on a page that is loaded, a row's arrow counted before and after, the
 * "Sidebar" list's other boxes beside the missing one. Everything here
...
const HOMEPAGE_ONLY = 'Display web feed links on homepage only.';
const BOOK_PAGE = 'catalog/book';
...
function makeTag(scenario, testInfo) {
    return `u18${scenario}omw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
/** A book submitted by the press's Author; returns its submission id. */
 ...
 * the second, so books seeded within one second tie in the recent list,
...
/** Expect each of the three feeds to list exactly `titles`, whatever the order (A8). */
async function expectFeedTitleSet(visitor, tag, titles) {
    const all = await titlesOfAll(visitor, tag);
    for (const type of FEED_TYPES) {
        expect([...all[type]].sort(), `the ${type} feed's items`).toEqual([...titles].sort());
    }
}

/** The three feed addresses of a press (paths), sorted as discoveryHrefs sorts them. */
...
test.describe('Web feeds', () => {
    test('S1: The three feeds of a journal with a published article', async ({ompApi, visitor}, testInfo) => {
        test.slow();
        const tag = makeTag('s1', testInfo);
        const other = `${tag}b`;
        await seedPress(ompApi, tag, {manager: false, context: {name: 'Sea Letters'}});
        const bookId = await seedBook(ompApi, tag, 'Tidal Patterns', {
        ...
        await seedBook(ompApi, other, 'Elsewhere', {published: true});
        const pages = new PublicPages(visitor, tag);
        const home = new RegExp(`/index\\.php/${tag}$`);
        const bookPage = new RegExp(`/index\\.php/${tag}/catalog/book/${bookId}$`);
        ...
        // Patterns" with the book's page, "Ada Author", the abstract and the
        // date (Rules 1, 7, 8; Fields).
        ...
        expect(atom.items[0].link).toMatch(bookPage);
        ...
        expect(rss2.items[0].link).toMatch(bookPage);
        ...
        expect(rdf.items[0].link).toMatch(bookPage);
        ...
        // The item's link: it opens the book's page of "Tidal Patterns" (Rule 7).
        await visitor.goto(atom.items[0].link);
        await expect(visitor).toHaveURL(bookPage);
        await expect(pages.mainHeading()).toHaveText('Tidal Patterns', {timeout: 30_000});
    ...
    test('S2: The feeds follow publishing', async ({asUser, ompApi, visitor}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        const tag = makeTag('s2', testInfo);
        await seedPress(ompApi, tag);
        const tidalId = await seedBook(ompApi, tag, 'Tidal Patterns', {published: true, datePublished: '2024-03-05'});
        await nextServerSecond(visitor.request);
        await seedBook(ompApi, tag, 'Coral Reefs', {published: true});
        ...
        // Published again: "Tidal Patterns" is listed again (Rule 3); on a
        // press its place is A8's, not asserted.
        await publishShown(page);
        await expectFeedTitleSet(visitor, tag, ['Tidal Patterns', 'Coral Reefs']);
        ...
        await openWorkflow(page, tag, tidalId, {menuKey: `publication_${newPubId}_titleAbstract`});
    ...
    test('S3: The "Latest publications" box, and the plugin switched off', async ({asUser, ompApi, visitor}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        const tag = makeTag('s3', testInfo);
        await seedPress(ompApi, tag, {plugins: {webfeedplugin: {enabled: true, settings: {recentItems: 5}}}});
        const bookId = await seedBook(ompApi, tag, 'Tidal Patterns', {published: true});
        const page = await actorPage(asUser, `${tag}mg`);
        const pages = new PublicPages(visitor, tag);
        const box = pages.box;
        ...
        // home page, "About the Press" and the book's page (Rule 11).
        for (const open of [() => pages.home(), () => pages.about(), () => pages.article(bookId, {pagePath: BOOK_PAGE})]) {
            await open();
    ...
    test('S4: Where the feeds are advertised', async ({asUser, ompApi, visitor}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        const book = () => pages.article(bookId, {pagePath: BOOK_PAGE});

        // The plugin's window: "Web Feed Plugin" ticked under "Generic
        // Plugins"; the window titled "Web Feed Plugin" with its
        // description, the heading "Settings", "…on homepage only." chosen,
        // 30, "Include identifiers…" unticked, "Cancel" and "OK" (Rule 1;
        // Fields).
        await plugins.goto();
        ...
        await expect(window.labelled('radio', HOMEPAGE_ONLY)).toBeChecked();
        ...
        // A new press's discovery links: the home page carries the three (Rule 12).
        await pages.home();
        expect(await discoveryHrefs(visitor)).toEqual(three);

        // Control: before the first "OK", "About the Press" and the book's
        // page carry no feed link (Rule 12).
        for (const open of [() => pages.about(), book]) {
            await open();
            await expect(discoveryLinks(visitor)).toHaveCount(0);
        }

        // "…on all application pages.": "OK" closes the window with "Your
        // changes have been saved."; About and the book's page now carry
    ...
    test('S5: Which articles, and how many', async ({asUser, ompApi, visitor}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        await seedBook(ompApi, tag, 'Alpha', {published: true, datePublished: '2024-03-03'});
        await nextServerSecond(visitor.request);
        await seedBook(ompApi, tag, 'Beta', {published: true, datePublished: '2024-03-02'});
        await nextServerSecond(visitor.request);
        await seedBook(ompApi, tag, 'Gamma', {
            published: true,
        ...
        // "Categories: Cat One, Cat Two" (a book in no series has no
        // "Series:" line), then an empty line and the abstract (Rule 9;
        // Settings bullet 5).
        await plugins.goto();
        let window = await plugins.openSettings();
    ...
    test('S6: The window\'s number box, "Cancel" and leaving unsaved', async ({asUser, ompApi}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        /** The window as a new press has it: the homepage choice, 30, the box unticked. */
    ...
    test('S7: A journal with nothing published, and the feed\'s description', async ({asUser, ompApi, visitor}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        // The press's summary replaces it, its tags written out [A6] (Rule
        // 8; Settings bullet 9).
        ...
        // The seeded press has no Country, which the tab requires before
    ...
    test('S8: A journal closed to visitors', async ({ompApi, visitor}, testInfo) => {
        test.slow();
        ...
        await seedBook(ompApi, tag, 'Tidal Patterns', {published: true});
        await seedPress(ompApi, other, {manager: false});
        await seedBook(ompApi, other, 'Open Study', {published: true});
        const pages = new PublicPages(visitor, tag);

        // Control: before the sign-in, the second press's Atom feed shows
