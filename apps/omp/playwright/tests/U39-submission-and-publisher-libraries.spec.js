 ...
 *
 * Submission & Publisher Libraries — OMP suite, one test per canonical
 * scenario the spec runs on a press (S1–S5: S1–S4 common, S5 {OJS OMP}),
 * in the press's own words: the "Press Library" (the Settings tab and the
 * list heading), the Press Manager, the Press Editor, a Series editor in
 * the Section Editor's part (S3), a monograph; "Contracts" heads every type
 * list (OMP1).
 ...
 *   press whose Copyeditor role has the Submission stage.
 ...
 * are read-only (A1, A7). S1 runs on publicknowledge on its own monograph
 * (a unique tag, M5; series `monographs`, whose submit-time
 * auto-assignment enrols `editor.diana`), with no downloaded name read and
 * nothing deleted;
 * S2–S5 run on scratch presses with throwaway accounts, as footnote s
 ...
 * submission; S5's second press ticks the Copyeditor's Submission stage
...
/** The press's library name: the Settings tab and the list heading (Rules 6, 9). */
const PUBLISHER_LIBRARY = 'Press Library';

/** The "Type" list's groups, in order (Fields "Type"). */
const TYPES = ['Contracts', 'Marketing', 'Permissions', 'Reports', 'Other'];
...
function makeTag(scenario, testInfo) {
    return `u39${scenario}ompw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
test.describe('submission and publisher libraries', () => {
    test('S1: a file shared through the Submission Library', {tag: '@smoke'}, async ({asUser, ompApi}, testInfo) => {
        test.slow();
        ...
        const ed = await pageAs(asUser, EDITOR, PRESS);
        ...
        await list.expectAllEmpty(['Contracts', 'Marketing', 'Reports', 'Other']);

        // ── The Author's list ────────────────────────────────────────────
        // From "My Submissions": "Library" lists "Author agreement" under
        // "Permissions" (Actors row 1; Rule 1).
        const au = await pageAs(asUser, AUTHOR, PRESS);
    ...
    test('S2: the Publisher Library on the Settings tab', async ({asUser, ompApi, browser, baseURL}, testInfo) => {
        test.slow();
        const tag = makeTag('s2', testInfo);
        const manager = `${tag}mg`;
        // Given: the Press Manager of a scratch press whose Publisher
        // Library (the Press Library) is empty (footnote s, recipe 2).
        await ompApi.createContext({tag, context: {name: `Press ${tag}`}, users: [user(manager, 'Mona', 'Manager', ['manager'])]});

        const mg = await pageAs(asUser, manager, tag);
        const tab = new PublisherLibraryTab(mg.page, tag, PUBLISHER_LIBRARY);

        // ── The tab ──────────────────────────────────────────────────────
        // "Workflow Settings" › "Press Library": the list headed so,
        ...
        // lines, "Replace file", "Public Access" ticked over the press's
    ...
    test('S3: the Publisher Library read from the workflow', async ({asUser, ompApi}, testInfo) => {
        test.slow();
        ...
        const author = `${tag}au`;
        // Given: a scratch press whose Press Library holds "Journal
        // guide" under "Other"; the Series editor (the Section Editor's part) assigned to a submission
        ...
        await ompApi.createContext({
            ...
            users: [
                ...
                user(sectionEditor, 'Sela', 'Series', ['sectionEditor']),
        ...
        // ── The Series editor's read-only list ──────────────────────────
        // "Library", "View Document Library": a second window over the
        // first, headed "Press Library", "Journal guide" under "Other";
        ...
        // ── The Series editor's list again ──────────────────────────────
        // (Rule 9)
        await vdl.close();
    ...
    test('S4: library files attached to a decision email', async ({asUser, ompApi, pkpMail, browser, baseURL}, testInfo) => {
        test.slow();
        ...
        // (footnote s, recipe 4).
        const context = await ompApi.createContext({
            tag,
            context: {name: `Press ${tag}`},
            users: [
                user(manager, 'Mona', 'Manager', ['manager']),
        ...
        const decided = await ompApi.createSubmission({
            tag: `${tag}d`,
            context: tag,
            submitter: author,
            title,
        ...
        const other = await ompApi.createSubmission({
            tag: `${tag}o`,
            context: tag,
            submitter: author,
            title: `Other ${tag}`,
    ...
    test('S5: an assistant on a stage it may not open', async ({asUser, ompApi}, testInfo) => {
        test.slow();
        const tag = makeTag('s5', testInfo);
        const tagB = `${tag}b`;
        // Given: on each of two scratch presses, a Copyeditor assigned to a
        // submission on the Production stage whose Library holds "Editor
        // contract" under "Permissions", the Press Library "Journal
        // guide" under "Other"; the second press's Copyeditor role with
        // "Submission" ticked under "Stage Assignment" (footnote s, recipe 5).
        const seed = async (press, extra = {}) => {
            await ompApi.createContext({
                tag: press,
                context: {name: `Press ${press}`},
                users: [
                    user(`${press}mg`, 'Mona', 'Manager', ['manager']),
                    user(`${press}ed`, 'Erin', 'Editor', ['editor']),
                    user(`${press}ce`, 'Cora', 'Copyeditor', ['copyeditor']),
                    user(`${press}au`, 'Ava', 'Author', ['author']),
                ],
                libraryFiles: [{name: 'Journal guide', type: 'Other'}],
                ...extra,
            });
            return await ompApi.createSubmission({
                ...
                context: press,
                submitter: `${press}au`,
                title: `Submission ${press}`,
                decisions: ['sendExternalReview', 'accept', 'sendToProduction'],
                participants: [{username: `${press}ce`, role: 'copyeditor'}],
        ...
        // On the second press: "Editor contract" downloads, the page
        ...
        // ── Control ──────────────────────────────────────────────────────
        // First press, Copyeditor: "View Document Library" lists "Journal
