 ...
 *
 * Users management — OMP suite, one test per canonical scenario the spec runs
 * on a press (scenarios 1–8, all common, in OMP vocabulary: press, Press
 * Manager, "Hosted Presses", "Press Registration", the reviewer roles
 ...
 * A2 🐞, A3 ❓, A4 🐞, A7 🐞, A8 ❓, A13 🐞. Where a test passes through one
 * (S4 reads the menus of rows outside the manager's reach, S3 opens "Enable
 * User", S5 reaches the removed user's roles page through her row, S7 reads
 * the grid's rows, S1 finds the search box by the start of its name, whose
 * example role is A4's) it asserts the effect the spec states and leaves
 * the finding's own claim unasserted either way.
 * The spec's Coverage section records everything else left out.
 *
 * Every test seeds its own scratch press with throwaway accounts (the
 * seeded roster and publicknowledge stay untouched); the Site Administrator
 * `admin` is enrolled as a manager of every scratch press and is the
...
const MANAGER_ROLE = 'Press manager';
const REVIEWER_ROLES = ['Internal Reviewer', 'External Reviewer'];
const NO_ROLE_BOX = 'Include users with no roles in this press.';
...
function makeTag(scenario, testInfo) {
    return `u53s${scenario}ompw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
test.describe('users management', () => {
    test('S2: email a user', async ({asUser, ompApi, pkpMail}, testInfo) => {
    ...
    test('S3: disable and re-enable a user', async ({asUser, ompApi, browser, baseURL}, testInfo) => {
        test.slow();
    ...
    test('S4: users outside the manager\'s reach', async ({asUser, ompApi}, testInfo) => {
    ...
    test('S5: remove a user from the press', async ({asUser, ompApi, browser, baseURL}, testInfo) => {
        ...
        // Control: the second press still lists her as Author (Rule 14).
    ...
    test('S6: merge a duplicate account', async ({asUser, ompApi, browser, baseURL}, testInfo) => {
        test.slow();
    ...
    test.describe('the Site Administrator\'s steps', () => {
        ...
        test('S1: find a user in the "Users" list', async ({asUser, ompApi}, testInfo) => {
            test.slow();
        ...
        test('S7: the Site Administrator adds a user', async ({asUser, ompApi, pkpMail, browser, baseURL}, testInfo) => {
            test.slow();
            ...
            // longer list (T-ojs-1; the same on a press).
            ...
            // server's refusals come at the next "OK" (T-ojs-2; the same on a press).
            const updates = [];
            ...
            for (const role of REVIEWER_ROLES) {
                await expect(add.mastheadBox(role)).toBeEnabled();
                await add.mastheadBox(role).uncheck();
                await expect(add.mastheadBox(role)).not.toBeChecked();
                await add.mastheadBox(role).check();
            }
            await add.pressSave();
            ...
            const summary = await pkpMail.find({to: rosaEmail, subject: 'Press Registration'});
            const message = await pkpMail.fullMessage(summary.ID);
        ...
        test('S8: the Site Administrator changes a user\'s roles on "Edit User"', async ({
            ...
            for (const role of REVIEWER_ROLES) {
                await expect(edit.mastheadBox(role)).toBeChecked();
                await expect(edit.mastheadBox(role)).toBeDisabled();
            }
