 ...
 *
 * Users management — OMP suite, the Site Administrator's notices: S7
 ...
 * two run alone in the `omp-solo` project (flake-s26 fixAD).
 *
 * Every test seeds its own scratch press with throwaway accounts; `admin`
...
test.describe('users management: the Site Administrator\'s notices', () => {
    test('S7: the Site Administrator adds a user @solo', async ({asUser, ompApi, pkpMail, browser, baseURL}, testInfo) => {
        test.slow();
