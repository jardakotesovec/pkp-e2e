 ...
 *
 * Notify users (bulk email) — OMP suite, the serial part: scenarios 1, 2, 4
 * and 5, each of which ends "once the site's background jobs have run",
 * in OMP vocabulary (press, Press Manager, "Hosted Presses", Settings ›
 * Press, whose page is headed "Setup", the wizard's first tab "Setup").
 ...
 * `@solo`: its Site Administrator unticks the press on Administration ›
 ...
 * press another worker seeds between that page's load and its "Save"
 * would be unticked with it. Alone in the `omp-solo` project nothing seeds
 ...
 *
 * Seeding (footnote s): a scratch press from `POST scenarios/context`
 * with `bulkEmails: true` and throwaway accounts (the username twice as
 * password, `<username>@mail.test`), the first a Press Manager. The
 * Site Administrator is `admin`, enrolled as a manager of every scratch
 * press. S1 names the press's principal contact, so the sender the
 * email carries is the test's own and is read back on Settings › Press ›
...
const NOT_ENABLED = 'The email notification feature has not been enabled for this press.';
const DISABLED_SENTENCE =
    'The bulk email feature has been disabled for this press. Enable this feature in Admin > Site Settings.';
...
test.describe('notify users (serial)', () => {
    test('S1: send an email to one role, then to two with a copy', async ({asUser, ompApi, pkpMail}, testInfo) => {
        ...
        // Control: Rui and the Press Manager have no "Office closed".
        for (const who of [rui, manager]) {
            await pkpMail.expectNone({to: mail(who), subject: 'Office closed', afterControl: {to: mail(quinn), subject: 'Office closed'}});
        }
    });

    test('S2: the roles offered, and a form refused, cancelled and left', async ({asUser, ompApi, pkpMail}, testInfo) => {
        ...
        await notify.settings.openSettingsEntry('Press');
        await expect(notify.settings.heading).toHaveText('Setup');
    ...
    test('S4: a role restricted while the manager\'s page is open', async ({asUser, ompApi, pkpMail}, testInfo) => {
        test.setTimeout(180_000);
    ...
    test('S5: bulk email withdrawn while the manager\'s page is open @solo', async ({asUser, ompApi, pkpMail}, testInfo) => {
        test.setTimeout(180_000);
        ...
        // The Site Administrator unticks the press (Settings bullet 1).
        ...
        // "Restrict Bulk Emails" on a press not allowed (Rule 11).
        const hosted = new HostedContextsPage(ap, LABELS);
        await hosted.gotoFromAdministration();
        await hosted.openSettingsWizard(tag);
        const restrict = new RestrictBulkEmailsTab(ap, WIZARD);
