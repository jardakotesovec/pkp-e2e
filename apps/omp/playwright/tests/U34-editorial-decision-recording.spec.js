 ...
 *
 * Editorial decision recording — OMP suite, one test per canonical scenario
 * the spec runs on a press, in the press's own words (Press Manager, Press
 * Editor, Series editor, monograph, "Send to External Review"): the eight
 * common scenarios (1–8, {OJS OMP}) and scenario 11 ({OMP} the press's
 * internal review decisions); scenario 9 is {OPS} and 10 {OJS}. The wizard
 * is one lib/pkp page on the three apps; the press's markers inside the
 * common scenarios (the Submission stage's "Send to External Review", the
 * author's task "Revisions to consider in External Review.", no "Request
 * Payment" page) are asserted where the scenario walks them.
 ...
 *   the button on a declined monograph only.
 * - A9 🐞 (a Series editor's "Find Template" refused): S2 searches as the
 *   Press Editor, whose search works.
 * - OMP1 🐞 (the press's "Review Cancel" email leaves "{$journalName}"
 *   unfilled): S6 reads the cancelled reviewer's email by its subject and
 *   sender; nothing reads its text.
 * - OJS1 🐞, OPS2 🐞, OPS1 ✅: other apps' territory.
 ...
 *   writer): S5 reads the Press Editor as the participant and the Series
 *   editor as "Created by" / "Message from", as the scenario states; nothing
 *   asserts who should be listed.
 ...
 * are read-only (A1, A7). S1, S2, S4 and S11 run on the seeded press with
 * scratch monographs (footnote s1: `author.alex` in series `monographs`
 * auto-assigns `editor.diana`); a monograph on an external round is
 * `['skipInternalReview']`, the press's Submission-stage "Send to External
 * Review", and one on an internal round `['sendInternalReview']` with
 * `reviewRounds: [{stage: 'internal', …}]`. S3, S5, S6, S7 and S8 run on
 ...
 * message that did arrive) or needs a setting at its non-default end
 * (`review` passthrough keys, the form languages). States with no seed key
 * are built on screen before the scenario's first step, as footnote s1
 * says: a submission file and a revision through the panels' "Upload", a
 * Library file through "Library" › "Add a file", a reviewer's file through
 * the reviewer wizard of a reviewer seeded `accepted` (a press's wizard has
 * no recommendation list), a confirmed review through "Read Review" › "Mark
 * as Complete", the recommend-only flag through the Participants row's
 * "Edit", a contributor without an account through "Contributors" › "Add
 * Contributor", an edited template and an alternative through Manage
 * Emails and the "Notify All Authors" choice through its settings screen
 * (`apps/omp/playwright/pages/DecisionSettingsPages.js`).
 * Tags are unique per run (M5); waits are web-first (A5). Everything runs
 * in the parallel `omp` project.
 */
...
const {DecisionWorkflow, roundTitle} = require('../pages/ReviewStagePages.js');
const {LISTS, selectStage, fileList, fileRows} = require('../pages/CopyeditingStagePages.js');
const {
    DecisionWizardPage,
    ComposerPage,
    CANCEL_DIALOG,
...
const {ManageEmailsPage, WorkflowEmailsSettingsPage} = require('../pages/DecisionSettingsPages.js');
...
/** The file the composer's "Upload File" attaches (app-local fixture). */
const FIXTURE_PDF = path.join(__dirname, '..', 'fixtures', 'files', 'article.pdf');
const FIXTURE_PDF_NAME = 'article.pdf';

/** The headings of a press's rounds. */
const EXTERNAL_ROUND_1 = roundTitle(1, 'External Review');
const EXTERNAL_ROUND_2 = roundTitle(2, 'External Review');
const INTERNAL_ROUND_1 = roundTitle(1, 'Internal Review');
...
const SENTENCE = {
    ...
    sendInternal: 'This submission is ready to be sent for internal review.',
    sendExternal: 'This submission is ready to be sent for peer review.',
};
...
/** The templates' default subjects (Side effects' table). */
const SUBJECT = {
    accepted: (press) => `Your submission has been accepted to ${press}`,
    ...
    sentForInternalReview: 'Your submission has been sent for internal review',
...
/** The author's task after "Request Revisions" on a press's external round (Side effects). */
const REVISION_TASK = 'Revisions to consider in External Review.';

/** The file type the press's upload wizard files a manuscript under. */
const MANUSCRIPT_TYPE = 'Book Manuscript';

/** Unique per-run tag: single alphanumeric token, app + scenario + worker. */
function makeTag(scenario, testInfo) {
    return `u34${scenario}ompw${testInfo.parallelIndex}${Math.random().toString(36).slice(2, 8)}`;
}
...
/** A page as a given user with the press's workflow page object and the wizard and composer readers. */
async function workflowAs(asUser, appContext, username, contextPath) {
    const context = await asUser(username);
    const page = await context.newPage();
    const workflow = new DecisionWorkflow(page, contextPath, {appContext});
    return {context, page, workflow, frame: workflow.frame, wizard: new DecisionWizardPage(page), composer: new ComposerPage(page)};
}

/**
 * Seed a monograph (Submission stage unless `decisions` say otherwise). On
 * the seeded press it rides series `monographs`, whose submit-time
 * auto-assignment enrols `editor.diana`; a scratch press has no series.
 */
async function seedMonograph(ompApi, tag, {context = PRESS, submitter = AUTHOR, title = `Submission ${tag}`, ...rest} = {}) {
...
/** Seed a monograph on its first external review round with these reviewers ("Send to External Review" from the Submission stage). */
async function seedInReview(ompApi, tag, {reviewers = [], reviewRounds = null, ...rest} = {}) {
    return await seedMonograph(ompApi, tag, {
        decisions: ['skipInternalReview'],
        reviewRounds: reviewRounds || [{stage: 'external', reviewers}],
        ...rest,
    });
}
...
/**
 * The scratch reviewer's wizard: the accepted seed's step 1, step 2, step 3
 * with a file, then "Submit Review" (a press's step 3 has no recommendation
 * list).
 */
async function reviewWithFile(page, contextPath, submissionId, fileName) {
    const wizard = new ReviewWizardPage(page, contextPath, {privateBoxLabel: 'For editor only'});
...
async function addContributor(page, {given, family, email}) {
    ...
    await contributors.addContributor({given, family, email, country: 'Canada', roles: ['Author']});
}

test.describe('editorial decision recording (U34)', () => {
    test('S1: accept a submission through the wizard', async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        const tag = makeTag('s1', testInfo);
        const title = `Submission ${tag}`;
        const revisionFile = `${tag}revision.txt`;
        ...
        const seeded = await seedInReview(ompApi, tag, {
            reviewers: [
                {username: 'reviewer.julia', status: 'completed'},
                {username: 'reviewer.paul', status: 'completed'},
            ],
        });
        const control = await seedInReview(ompApi, `${tag}c`, {reviewers: [{username: 'reviewer.julia'}]});

        const {page, workflow, frame, wizard, composer} = await workflowAs(asUser, appContext, EDITOR, PRESS);
        await workflow.gotoEditorial(seeded.submissionId);
        await workflow.expectPageTitle(EXTERNAL_ROUND_1);
        await workflow.uploadRevision(revisionFile);
        await page.reload();
        ...
        await expect(composer.subjectInput()).toHaveValue(SUBJECT.accepted(PRESS_NAME), {timeout: 30_000});
        expect(await composer.firstParagraphText()).toBe(`Dear ${AUTHOR_NAME},`);
        expect(await composer.letterText()).toContain(PRESS_NAME);
        ...
        const revisionRow = wizard.fileRow('Revisions', revisionFile);
        await expect(revisionRow).toHaveCount(1);
        await expect(wizard.promoteFileCheckbox(revisionFile)).toBeChecked();
        await expect(revisionRow).toContainText(MANUSCRIPT_TYPE);
        // The uploader line names the account by username (the OJS suite's finding T-ojs-2 holds on a press).
    ...
    test('S2: the composer: "Insert Content", "Attach Files", the templates and the refusals', async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        // leaves the list whole: the OJS suite's finding T-ojs-1 holds on a press).
    ...
    test("S3: request revisions with a reviewer's file attached", async ({asUser, ompApi, appContext, pkpMail}, testInfo) => {
        test.slow();
        ...
        // Given: a scratch press whose default review type is Open; the
        ...
        const first = await seedInReview(ompApi, tag, {
            context: tag,
            submitter: author,
            title: `First ${tag}`,
            reviewers: [
        ...
        const second = await seedInReview(ompApi, `${tag}b`, {
            context: tag,
            submitter: author,
            title: `Second ${tag}`,
        ...
        const {page, workflow, frame, wizard, composer} = await workflowAs(asUser, appContext, editor, tag);
        await workflow.gotoEditorial(first.submissionId);
        await workflow.expectPageTitle(EXTERNAL_ROUND_1);
        await workflow.confirmReview(RV1_DISPLAY);
        const libraryType = await workflow.addLibraryFile({name: libraryName, file: FIXTURE_PDF});
        await page.reload();
        await workflow.expectOpen();
        ...
        await expect(libraryFiles).toContainText(libraryType);
        ...
        // header's Tasks panel reads "Revisions to consider in External Review."
        // (the press's words) linking to the
        // submission; the "Read Review" window of the completed review
        // lists the reviewer's file under "Reviewer Files" (Rule 6; Side
        // effects).
        const authorSide = await workflowAs(asUser, appContext, author, tag);
        await authorSide.workflow.gotoAuthor(first.submissionId);
        await authorSide.workflow.expectPageTitle(EXTERNAL_ROUND_1);
        ...
        await tasks.open();
        const revisionTask = tasks.row(REVISION_TASK).filter({hasText: firstTitle});
        ...
        await expect(tasks.row(REVISION_TASK).filter({hasText: firstTitle})).toHaveCount(1);
        await tasks.close();
    });

    test('S4: the wizard refuses the wrong hands, the wrong stage and a stale page', async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        const tag = makeTag('s4', testInfo);
        // Given: the Editor auto-assigned to a submission at the Submission
        // stage; the Copyeditor, the Author, a Series editor of another
        // series and the Press Manager on the seeded press (footnote s1).
        const seeded = await seedMonograph(ompApi, tag);
        const declineUrl = DecisionWizardPage.recordUrl(PRESS, seeded.submissionId, DECISION_DECLINE);
        const productionUrl = DecisionWizardPage.recordUrl(PRESS, seeded.submissionId, DECISION_SEND_TO_PRODUCTION);

        // ── The Copyeditor, the Author and the unassigned Series editor by address ──
        ...
        for (const [username, message] of [
            ...
            const other = await workflowAs(asUser, appContext, username, PRESS);
        ...
        const editor = await workflowAs(asUser, appContext, EDITOR, PRESS);
        ...
        expect(opened.pathname).toMatch(new RegExp(`^/index.php/${PRESS}(/en)?/decision/record/${seeded.submissionId}$`));
        expect(opened.searchParams.get('decision')).toBe(String(DECISION_DECLINE));

        // ── The Press Manager, not assigned ────────────────────────────
        // In a session of their own the Manager opens the same submission,
        // presses "Send to External Review" (the press's "Send for Review")
        // and records it, no assignment needed
        // (Actors row 1).
        const manager = await workflowAs(asUser, appContext, 'manager.maya', PRESS);
        ...
        await manager.frame.actionButton('Send to External Review').click();
        await manager.wizard.expectTitle('Send to External Review: Notify Authors');
        await manager.wizard.continueStep();
        await expect(manager.wizard.stepHeading('Select Files')).toBeVisible({timeout: 30_000});
        const sent = await manager.wizard.recordDecision('Sent for External Review');
        await expect(sent).toContainText(`The submission, Submission ${tag}, has been sent to the external review stage.`);
        await manager.wizard.viewSubmissionSummary();
        await manager.frame.expectOpen(seeded.submissionId);
        await manager.frame.expectStage(EXTERNAL_ROUND_1);
    ...
    test('S5: record a recommendation, then change it', async ({asUser, ompApi, appContext, pkpMail}, testInfo) => {
        test.slow();
        ...
        // Given: a scratch press; the first submission with the Editor and
        // the Series editor assigned to its round with one review
        // submitted; the second with the Series editor alone; the
        ...
        const ed = await workflowAs(asUser, appContext, editor, tag);
        ...
        const se = await workflowAs(asUser, appContext, sectionEditor, tag);
        await se.workflow.gotoEditorial(first.submissionId);
        await se.workflow.expectPageTitle(EXTERNAL_ROUND_1);
        ...
        await se.frame.expectStage(EXTERNAL_ROUND_1);
        ...
        // as its participant and the Series editor only as "Created by"
        // (A4 ❓, read as the scenario states); the Editor's mailbox holds
        // "Editor Recommendation" from the Series editor (Rule 13; Side
        // effects; Actors row 10).
        await ed.workflow.gotoEditorial(first.submissionId);
        await ed.workflow.expectPageTitle(EXTERNAL_ROUND_1);
        ...
        // ── Control ──────────────────────────────────────────────────────
        // On the first submission the Series editor's round offers no
    ...
    test('S6: accept with the reviewers notified, below and at the minimum', async ({asUser, ompApi, appContext, pkpMail}, testInfo) => {
        test.slow();
        test.setTimeout(420_000);
        ...
        // Given: a scratch press whose "Minimum Confirmed Reviews Required"
        ...
        const first = await seedInReview(ompApi, tag, {context: tag, submitter: author, title: `First ${tag}`, reviewers: two, participants});
        const second = await seedInReview(ompApi, `${tag}b`, {context: tag, submitter: author, title: `Second ${tag}`, reviewers: two, participants});
        const third = await seedInReview(ompApi, `${tag}c`, {
            context: tag,
            submitter: author,
            title: `Third ${tag}`,
        ...
        await frame.expectOpen(first.submissionId);
        await frame.expectStage(EXTERNAL_ROUND_1);
        await frame.openActivityLog();
        ...
        await frame.expectOpen(first.submissionId);
        await frame.expectStage('Copyediting');
        await selectStage(frame.dialog(), 'Copyediting');
        await expect(fileList(frame.dialog(), LISTS.draft)).toBeVisible({timeout: 30_000});
        await expect(fileList(frame.dialog(), LISTS.draft).getByRole('cell', {name: 'No Items', exact: true})).toBeVisible({timeout: 30_000});
        await expect(fileRows(frame.dialog(), LISTS.draft)).toHaveCount(0);
        ...
        await workflow.gotoEditorial(second.submissionId);
        await workflow.expectPageTitle(EXTERNAL_ROUND_1);
        await workflow.confirmReview(names[rv1]);
        await workflow.confirmReview(names[rv2]);
        await frame.actionButton('Accept Submission').click();
        await wizard.expectTitle('Accept Submission: Notify Authors');
        await expect(workflow.minimumReviewsDialog()).toHaveCount(0);
        ...
        await workflow.expectPageTitle(EXTERNAL_ROUND_2);
    ...
    test("S7: decline: the author's email, the contributor's copy, then a skipped revert", async ({asUser, ompApi, appContext, pkpMail}, testInfo) => {
        test.slow();
        test.setTimeout(420_000);
        ...
        const title = `Submission ${tag}`;
        const submissionFile = `${tag}manuscript.txt`;
        ...
        // Given: a scratch press whose decline template is edited and
        ...
        await workflow.gotoEditorial(seeded.submissionId);
        await workflow.expectPageTitle('Submission');
        await workflow.uploadSubmissionFile(submissionFile);
        await addContributor(page, {given: 'Cleo', family: 'Contributor', email: contributorEmail});
        ...
        await expect(submissionFiles).toContainText(submissionFile);
        await composer.attachSelected(submissionFiles, submissionFile, submissionFile);
        await expect(composer.attachmentChips()).toHaveCount(1);
        ...
        expect((declineFull.Attachments || []).map((a) => a.FileName)).toEqual([submissionFile]);

        // ── The contributor's mailbox ────────────────────────────────────
        // An email under the same subject, opening "The following email was
        // sent to {author} from {press} regarding "{title}"." and quoting
        ...
        expect(copyFull.Text.trim().startsWith(`The following email was sent to ${AUTHOR_DISPLAY} from ${pressName} regarding "${title}".`)).toBe(true);
    ...
    test('S8: decline in French, to the assigned authors alone', async ({asUser, ompApi, appContext, pkpMail}, testInfo) => {
        test.slow();
        test.setTimeout(360_000);
        ...
        const contributorEmail = `${tag}co@mail.test`;
        const title = `Submission ${tag}`;
        // Given: a scratch press with French as a second form language and
        ...
        await ompApi.createContext({
            ...
            // answers 400 "This language is not accepted."; the OJS suite's
            // T-ojs-4, which holds on a press: T-omp-1, a
        ...
        const control = await seedMonograph(ompApi, `${tag}c`);
        expect(await pkpMail.count({to: mailOf(author)})).toBe(0);
        ...
        // ── Control ──────────────────────────────────────────────────────
        // On the seeded press, whose forms have one language, the wizard
        // shows no "Switch to:" line under the templates, the templates
        // heading being there (Rule 8; Settings bullet 3).
        const seededSide = await workflowAs(asUser, appContext, EDITOR, PRESS);
    ...
    test("S11: the press's internal review decisions", async ({asUser, ompApi, appContext}, testInfo) => {
        test.slow();
        test.setTimeout(300_000);
        ...
        const firstTitle = `First ${tag}`;
        const secondTitle = `Second ${tag}`;
        // Given: the Press Editor auto-assigned to three monographs on the
        // seeded press: one at the Submission stage, its one submission file
        // uploaded on screen below; one on an internal review round with one
        // review submitted; one on an external review round with one review
        // submitted (footnote s1).
        const first = await seedMonograph(ompApi, tag, {title: firstTitle});
        ...
        // The wizard headed "Send to Internal Review: Notify Authors" with
        // its sentence and the rail "1 Notify Authors", "2 Select Files";
        // "Email Templates" lists "Sent to Internal Review" and "Subject:"
        // reads its subject; "Continue": the panel "Select Files" with "Select
        // files that should be sent to the internal review stage." and the
        // list "Submission Files" holding the file, ticked; recorded: the
        // window "Sent for Internal Review" with its sentence; the Activity
        // Log's line (Rules 2, 9, 11; Side effects).
        await frame.actionButton('Send to Internal Review').click();
        await wizard.expectTitle('Send to Internal Review: Notify Authors');
        await expect(wizard.pageDescription()).toHaveText(SENTENCE.sendInternal);
        await wizard.expectSteps(['Notify Authors', 'Select Files']);
        await wizard.expectStepCurrent('Notify Authors', 1);
        await wizard.expectStepUnreached('Select Files', 2);
        await wizard.awaitComposerLoaded();
        await composer.expectTemplates(['Sent to Internal Review']);
        await expect(composer.subjectInput()).toHaveValue(SUBJECT.sentForInternalReview, {timeout: 30_000});
        await wizard.continueStep();
        await wizard.expectTitle('Send to Internal Review: Select Files');
        await expect(wizard.stepHeading('Select Files')).toBeVisible({timeout: 30_000});
        await expect(wizard.stepDescription()).toHaveText('Select files that should be sent to the internal review stage.');
        await expect.poll(() => wizard.filesListTitles(), {timeout: 30_000}).toEqual(['Submission Files']);
        await expect(wizard.fileRow('Submission Files', submissionFile)).toHaveCount(1);
        await expect(wizard.promoteCheckboxes()).toHaveCount(1);
        await expect(wizard.promoteFileCheckbox(submissionFile)).toBeChecked();
        const sentInternal = await wizard.recordDecision('Sent for Internal Review');
        await expect(sentInternal).toContainText(
            `The submission, ${firstTitle}, has been sent to the internal review stage. The author has been notified, unless you chose to skip that email.`
        );
        await wizard.viewSubmissionSummary();
        await frame.expectOpen(first.submissionId);
        await frame.expectStage(INTERNAL_ROUND_1);
        await frame.openActivityLog();
        await expect(frame.activityLogRow(`${EDITOR_NAME} sent this submission to the internal review stage.`)).toHaveCount(1, {
            timeout: 30_000,
        });
        await frame.closeActivityLog();

        // ── "Request Revisions" on the internal round ────────────────────
        // On the second monograph "Request Revisions": no choice window;
        // the wizard opens at once headed "Request Revisions: Notify
        // Authors" (the opened wizard is the positive read; the external
        // round's window is the Control below); "Cancel" and "Cancel
        // Decision" (Rules 11, 14).
        await workflow.gotoEditorial(second.submissionId);
        await workflow.expectPageTitle(INTERNAL_ROUND_1);
        await expect(workflow.reviewerRow('Amara Reviewer')).toContainText('Review Submitted', {timeout: 30_000});
        await frame.actionButton('Request Revisions').click();
        await wizard.expectTitle('Request Revisions: Notify Authors');
        await expect(workflow.requestRevisionsWindow()).toHaveCount(0);
        await expect(wizard.pageDescription()).toHaveText(SENTENCE.requestRevisions);
        await wizard.awaitComposerLoaded();
        const cancelDialog = await wizard.pressCancel();
        await expect(cancelDialog).toContainText(CANCEL_QUESTION);
        ...
        // ── "Send to External Review" from the internal round ────────────
        // The wizard headed "Send to External Review: Notify Authors" with
        // its sentence and the rail "1 Notify Authors", "2 Select Files";
        // "Email Templates" lists "Sent to Review"; recorded: the window
        // "Sent for External Review" with its sentence; the Activity Log's
        // line (Rules 2, 11; Side effects).
        await frame.actionButton('Send to External Review').click();
        await wizard.expectTitle('Send to External Review: Notify Authors');
        await expect(wizard.pageDescription()).toHaveText(SENTENCE.sendExternal);
        await wizard.expectSteps(['Notify Authors', 'Select Files']);
        await expect(wizard.stepItem('Notify Reviewers')).toHaveCount(0);
        await wizard.awaitComposerLoaded();
        await composer.expectTemplates(['Sent to Review']);
        await expect(composer.subjectInput()).toHaveValue(SUBJECT.sentForReview, {timeout: 30_000});
        await wizard.continueStep();
        await wizard.expectTitle('Send to External Review: Select Files');
        await expect(wizard.stepHeading('Select Files')).toBeVisible({timeout: 30_000});
        const sentExternal = await wizard.recordDecision('Sent for External Review');
        await expect(sentExternal).toContainText(`The submission, ${secondTitle}, has been sent to the external review stage.`);
        await wizard.viewSubmissionSummary();
        await frame.expectOpen(second.submissionId);
        await frame.expectStage(EXTERNAL_ROUND_1);
        await frame.openActivityLog();
        await expect(frame.activityLogRow(`${EDITOR_NAME} sent this submission to the external review stage.`)).toHaveCount(1, {
            timeout: 30_000,
        });
        await expect(frame.activityLogRow('requested revisions for this submission')).toHaveCount(0);
        await frame.closeActivityLog();

        // ── Control ──────────────────────────────────────────────────────
        // On the third monograph "Request Revisions" opens the side window
        // "Request Revisions" with "Require New Review Round" first, and its
        // close control abandons the choice with nothing opened (Rule 14).
        await workflow.gotoEditorial(third.submissionId);
        await workflow.expectPageTitle(EXTERNAL_ROUND_1);
        const choice = await workflow.openRequestRevisionsWindow();
        await expect(choice).toContainText('Require New Review Round');
        ...
        await expect(workflow.revisionOption(choice, 'Revisions will not be subject to a new round of peer reviews.')).toBeChecked();
        await expect(wizard.heading()).toHaveCount(0);
        await workflow.closeRequestRevisionsWindow(choice);
        await frame.expectOpen(third.submissionId);
        await frame.expectStage(EXTERNAL_ROUND_1);
        await expect(frame.actionButton('Request Revisions')).toBeVisible();
        await expect(wizard.heading()).toHaveCount(0);
        expect(new URL(page.url()).pathname).not.toContain('/decision/record/');
    });
});
