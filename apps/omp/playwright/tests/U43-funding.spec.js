 ...
 *
 * Funding — OMP suite, one test per canonical COMMON scenario the spec runs
 * on a press (scenarios 1–4, in OMP vocabulary: press, monograph, catalog
 * book page) plus the OMP-side read-only rule (Rule 8 / Actors &
 * permissions — the press author's workflow list is read-only, the
 * counterpart of the spec's OPS1 contrast). The press-specific markers of
 * Rule 10 ride inside S2: the wizard's Funders section is NOT the Details
 * step's last section on a press — Chapters follow it.
 * Spec: docs/specs/U43-funding.md
 *
 * Deliberately NOT covered (register IDs from the spec's Findings register):
 * - The registry-pick leg of scenario 1 (a funder fixed to its ROR registry
 *   record, the row's ROR mark, the catalog page's registry link): on this
 ...
 *   the resumable wizard and in the workflow instead. Observed while
 *   building this suite: without that reload the Review step ALSO reads
 *   "None provided" on OMP — the staleness covers the wizard's whole
 *   in-memory publication state, not just the section's table (reported
 *   to the spec's register; not asserted here either way).
 ...
 * are read-only (the settings mutation of S4 runs on a scratch press with
...
test.describe('Funding (U43)', () => {
    test('S1: record and revise funding in the workflow', {tag: '@smoke'}, async ({asUser, ompApi}, testInfo) => {
        test.slow();
    ...
    test('S2: declare funding while submitting', {tag: '@smoke'}, async ({asUser, ompApi}, testInfo) => {
        ...
        // Rule 10, press marker: the Funders section is not the step's last
        // — Chapters follow it (visible section headings, in DOM order).
        ...
        // level 2 — the chapters grid nests its own h4 "Chapters" heading.
        await expect(
            authorPage.getByRole('heading', {name: 'Chapters', exact: true, level: 2})
        ).toBeVisible();
        ...
        // Add the funder through the same panel as the workflow. The
        // section's table does not refresh on a press (A4 🐞) — nothing is
        // asserted about it after the save; the save itself is bounded by
        // the funders API response and the panel closing.
        ...
        // The Press Manager finds the author's funder on the workflow's
    ...
    test('S3: readers see a published book\'s funding', async ({asUser, ompApi, page}, testInfo) => {
        test.slow();
        ...
        // The manager records the funder on the published monograph
        // (publishing does not lock the list — Actors & permissions) with a
        // fully filled grant. Typed-name path: on this install a
        // registry-backed funder cannot be produced (see the file header),
        // so the ROR-mark link of Rule 9 stays uncovered.
        const managerPage = await (await asUser('manager.maya')).newPage();
        ...
        // Rule 9, presence: on a press the landing page is the catalog's
        // book page. The anonymous reader's page carries the Funders block
        // with the funder's name and the grant's name, number and DOI (a
        // working doi.org link).
        await page.goto(
            `/index.php/${PK}${PK_PREFIX}/catalog/book/${withFunder.submissionId}`
        );
        ...
        // Rule 9, absence: a published book with no funders shows no
        // Funders block at all — bounded by the page having rendered
        // (its title), the same way the presence leg was read.
        await page.goto(
            `/index.php/${PK}${PK_PREFIX}/catalog/book/${noFunder.submissionId}`
        );
        await expect(
    ...
    test('S4: the press opts out of funding', async ({asUser, ompApi}, testInfo) => {
        ...
        // Scratch press (the setting is mutated) with throwaway users.
        ...
        // A funder recorded while the press sits at its "ask" default —
        ...
        // the step itself being current (its Chapters section renders;
        // level 2 — the chapters grid nests its own h4 "Chapters" heading).
    ...
    test('OMP-A: the author\'s workflow funding list is read-only', async ({asUser, ompApi}, testInfo) => {
        test.slow();
